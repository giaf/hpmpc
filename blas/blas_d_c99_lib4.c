...
void dgemm_ppp_nt_lib(int m, int n, int k, double *pA, int sda, double *pB, int sdb, double *pC, int sdc, int alg)
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_pp_nt_4x4_c99_lib4(k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, alg);
			}
		jj = 0;
		for(; jj<n-j-1; jj+=2)
			{
...
void dtrmm_ppp_lib(int m, int n, int offset, double *pA, int sda, double *pB, int sdb, double *pC, int sdc)
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_pp_nt_4x4_c99_lib4(n-j-0, &pA[0+(j+0)*bs+i*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, 0);
			}
		if(n-j==1)
			{
...
void dpotrf_p_dcopy_p_t_lib(int n, int nna, double *pC, int sdc, double *pL, int sdl)
	...
	if(j<nna-3)
		...
		for(; j<nna-3; j+=4)
			...
			for(; i<n; i+=4)
				{
				kernel_dgemm_pp_nt_4x4_c99_lib4(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
				}
			kernel_dpotrf_dtrsv_4x4_c99_lib4(n-j-4, &pC[0+j*bs+j*sdc], sdc);
			}
		}
