...
void d_back_ric_rec_sv_libstr(int N, int *nx, int *nu, int *nb, int **hidxb, int *ng, int update_b, struct d_strmat *hsBAbt, struct d_strvec *hsb, int update_q, struct d_strmat *hsRSQrq, struct d_strvec *hsrq, struct d_strvec *hsdRSQ, struct d_strmat *hsDCt, struct d_strvec *hsQxb, struct d_strvec *hsQxg, struct d_strvec *hsqxb, struct d_strvec *hsqxg, struct d_strvec *hsux, int compute_pi, struct d_strvec *hspi, int compute_Pb, struct d_strvec *hsPb, struct d_strmat *hsL, struct d_strmat *hsLxt, struct d_strmat *hswork_mat, struct d_strvec *hswork_vec)
	{
	...
	if(nb[N]>0)
		{
		ddiaadin_libspstr(nb[N], hidxb[N], 1.0, &hsQxb[N], 0, &hsdRSQ[N], 0, &hsRSQrq[N], 0, 0);
		drowad_libspstr(nb[N], hidxb[N], 1.0, &hsqxb[N], 0, &hsRSQrq[N], nx[N], 0);
		}
	if(ng[N]>0)
		{
	...
	for(nn=0; nn<N; nn++)
		...
		if(compute_Pb)
			...
			dtrmv_utn_libstr(nx[N-nn], &hsLxt[N-nn], 0, 0, &hswork_vec[0], 0, &hsPb[N-nn-1], 0);
			}
		...
		if(nb[N-nn-1]>0)
			{
			ddiaadin_libspstr(nb[N-nn-1], hidxb[N-nn-1], 1.0, &hsQxb[N-nn-1], 0, &hsdRSQ[N-nn-1], 0, &hsRSQrq[N-nn-1], 0, 0);
			drowad_libspstr(nb[N-nn-1], hidxb[N-nn-1], 1.0, &hsqxb[N-nn-1], 0, &hsRSQrq[N-nn-1], nu[N-nn-1]+nx[N-nn-1], 0);
			}
		if(ng[N-nn-1]>0)
			{
			printf("\nfeature not implemented yet\n\n");
...
void d_back_ric_trf_libstr(int N, int *nx, int *nu, int *nb, int **hidxb, int *ng, struct d_strmat *hsBAbt, struct d_strmat *hsRSQrq, struct d_strvec *hsdRSQ, struct d_strmat *hsDCt, struct d_strvec *hsQxb, struct d_strvec *hsQxg, struct d_strmat *hsL, struct d_strmat *hsLxt, struct d_strmat *hswork_mat)
	{
...
void d_back_ric_rec_trs_libstr(int N, int *nx, int *nu, int *nb, int **idxb, int *ng, struct d_strmat *hsBAbt, struct d_strvec *hsb, struct d_strvec *hsrq, struct d_strmat *hsDCt, struct d_strvec *hsqxb, struct d_strvec *hsqxg, struct d_strvec *hsux, int compute_pi, struct d_strvec *hspi, int compute_Pb, struct d_strvec *hsPb, struct d_strmat *hsL, struct d_strmat *hsLxt, struct d_strvec *hswork_vec)
	{
	...
	if(nb[N]>0)
		{
		dvecad_libspstr(nb[N], idxb[N], 1.0, &hsqxb[N], 0, &hsux[N], 0);
		}
	// general constraints
	if(ng[N]>0)
		{
		dgemv_n_libstr(nx[N], ng[N], 1.0, &hsDCt[N], 0, 0, &hsqxg[N], 0, 1.0, &hsux[N], 0, &hsux[N], 0);
		}
	...
	for(nn=0; nn<N-1; nn++)
		...
		if(nb[N-nn-1]>0)
			{
			dvecad_libspstr(nb[N-nn-1], idxb[N-nn-1], 1.0, &hsqxb[N-nn-1], 0, &hsux[N-nn-1], 0);
			}
		if(ng[N-nn-1]>0)
			{
			dgemv_n_libstr(nu[N-nn-1]+nx[N-nn-1], ng[N-nn-1], 1.0, &hsDCt[N-nn-1], 0, 0, &hsqxg[N-nn-1], 0, 1.0, &hsux[N-nn-1], 0, &hsux[N-nn-1], 0);
			}
