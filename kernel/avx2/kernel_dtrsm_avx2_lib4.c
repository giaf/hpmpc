...
void kernel_dgemm_dtrsm_nt_12x4_vs_lib4(int km, int kn, int tri, int kadd, int ksub, double *Ap0, int sdap, double *Bp, double *Am0, int sdam, double *Bm, double *C0, int sdc, double *D0, int sdd, double *fact, int alg)
	{
...
//			Bp += 4; // keep it !!!
	...
	a_00 = _mm256_broadcast_sd( &fact[0] );
	d_00 = _mm256_mul_pd( d_00, a_00 );
	_mm256_maskstore_pd( &D[0+bs*0], mask_m, d_00 );

	a_10 = _mm256_broadcast_sd( &fact[1] );
	a_11 = _mm256_broadcast_sd( &fact[2] );
	d_01 = _mm256_fnmadd_pd( d_00, a_10, d_01 );
	d_01 = _mm256_mul_pd( d_01, a_11 );
	_mm256_maskstore_pd( &D[0+bs*1], mask_m, d_01 );
	...
	d_02 = _mm256_fnmadd_pd( d_01, a_21, d_02 );
	d_02 = _mm256_mul_pd( d_02, a_22 );
	_mm256_maskstore_pd( &D[0+bs*2], mask_m, d_02 );
	...
	if(kn>=4)
		...
		d_03 = _mm256_fnmadd_pd( d_02, a_32, d_03 );
		d_03 = _mm256_mul_pd( d_03, a_33 );
		_mm256_maskstore_pd( &D[0+bs*3], mask_m, d_03 );
		}
...
//			Bp += 4; // keep it !!!
	...
	if(km>=2)
		...
		if(kn>=2)
			...
			a_10 = _mm_load_pd( &fact[1] );
			a_11 = _mm_load_pd( &fact[2] );
			d_01_11 = _mm_fnmadd_sd( d_00_10, a_10, d_01_11 );
			d_01_11 = _mm_mul_sd( d_01_11, a_11 );
			_mm_store_sd( &D[0+bs*1], d_01_11 );
			}
		}
