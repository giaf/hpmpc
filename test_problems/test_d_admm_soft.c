...
/*	printf("\nflush subnormals to zero\n");*/
	...
	printf("\n");
	printf(" ADMM method parameters: double precision, %d maximum iterations, %5.1e exit tolerance in primal and duality measure (edit file test_admm_ip_box.c to change them).\n", K_MAX_ADMM, TOL);
...
/*		db[jj] = - 4.0;   // xmin*/
	...
	double *lb; d_zeros_align(&lb, nx+nu, 1);
	for(jj=0; jj<nu; jj++)
		lb[jj] = - 0.5;   // umin
	for(; jj<nu+nx; jj++)
		lb[jj] = - 1.0;   // xmin

	double *ub; d_zeros_align(&ub, nx+nu, 1);
	for(jj=0; jj<nu; jj++)
		ub[jj] = 0.5;   // uman
	for(; jj<nu+nx; jj++)
		ub[jj] = 1.0;   // xman
...
************************************************/
	...
	double *work; d_zeros_align(&work, (N+1)*(pnz*cnl + 6*anz + 11*anx) + 3*anz, 1); // work space
...
/*	double sp_thr = SP_THR; // threshold to switch between double and single precision*/
	...
	double tol = TOL*sqrt(N*(nx+nu));//TOL; // tolerance in the duality measure
...
/*	double mu = -1.0;*/
	...
	if(FREE_X0==0)
		{
		d_admm_soft_mpc(&kk, k_max, tol, tol, warm_start, 1, rho, alpha, stat, nx, nu, N, hpBAbt, hpQ, hpS, hlb, hub, hux, compute_mult, hpi, work);
		}
	else
		{
/*		d_ip_box_mhe(&kk, k_max, tol, warm_start, sigma, stat, nx, nu, N, nb, hpBAbt, hpQ, hdb, hux, compute_mult, hpi, hlam, ht, work);*/
	...
	for(rep=0; rep<nrep; rep++)
		...
		if(FREE_X0==0)
			{
			d_admm_soft_mpc(&kk, k_max, tol, tol, warm_start, 0, rho, alpha, stat, nx, nu, N, hpBAbt, hpQ, hpS, hlb, hub, hux, compute_mult, hpi, work);
			}
		else
			{
/*			d_ip_box_mhe(&kk, k_max, tol, warm_start, sigma, stat, nx, nu, N, nb, hpBAbt, hpQ, hdb, hux, compute_mult, hpi, hlam, ht, work);*/
