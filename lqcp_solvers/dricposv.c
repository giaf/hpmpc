...
void dricposv_mpc(int nx, int nu, int N, double **hpBAbt, double **hpQ, double **hux, double **hpL, double *work, double *diag, int compute_pi, double **hpi)
	...
	// final stage 
/*	dsyrk_dpotrf_pp_lib(nz, 0, nz, hpL[N]+(nx+pad)*bs, 2*sda, hpQ[N], sda, diag);*/
	dsyrk_dpotrf_pp_lib(nx+nu%bs+1, 0, nx+nu%bs, hpL[N]+(nx+pad)*bs+(nu/bs)*bs*(2*sda)+(nu/bs)*bs*bs, 2*sda, hpQ[N]+(nu/bs)*bs*sda+(nu/bs)*bs*bs, sda, diag);

/*d_print_pmat(nz, nz, bs, hpL[N]+(nx+pad)*bs, 2*sda);*/

	d_transpose_pmat_lo(nx, nu, hpL[N]+(nx+pad)*bs+(nu/bs)*bs*(2*sda)+nu%bs+nu*bs, 2*sda, hpL[N]+(nx+pad+d_ncl)*bs, 2*sda);

/*d_print_pmat(nz, nz, bs, hpL[N]+(nx+pad+d_ncl)*bs, 2*sda);*/
/*exit(2);*/

	// middle stages 
	for(ii=0; ii<N-1; ii++)
		{	
		dtrmm_ppp_lib(nz, nx, hpBAbt[N-ii-1], sda, hpL[N-ii]+(nx+pad+d_ncl)*bs, 2*sda, hpL[N-ii-1], 2*sda); // TODO allow 'rectanguar' B
		for(jj=0; jj<nx; jj++) hpL[N-ii-1][((nx+nu)/bs)*bs*(2*sda)+(nx+nu)%bs+jj*bs] += hpL[N-ii][((nx+nu)/bs)*bs*2*sda+(nx+nu)%bs+(nx+pad+nu+jj)*bs];
		dsyrk_dpotrf_pp_lib(nz, nx, nu+nx, hpL[N-ii-1], 2*sda, hpQ[N-ii-1], sda, diag);
		for(jj=0; jj<nu; jj++) hpL[N-ii-1][(nx+pad)*bs+(jj/bs)*bs*(2*sda)+jj%bs+jj*bs] = diag[jj]; // copy reciprocal of diagonal
		d_transpose_pmat_lo(nx, nu, hpL[N-ii-1]+(nx+pad)*bs+(nu/bs)*bs*(2*sda)+nu%bs+nu*bs, 2*sda, hpL[N-ii-1]+(nx+pad+d_ncl)*bs, 2*sda);
		}

	// first stage 
	dtrmm_ppp_lib(nz, nx, hpBAbt[0], sda, hpL[1]+(nx+pad+d_ncl)*bs, 2*sda, hpL[0], 2*sda);
	for(jj=0; jj<nx; jj++) hpL[0][((nx+nu)/bs)*bs*(2*sda)+(nx+nu)%bs+jj*bs] += hpL[1][((nx+nu)/bs)*bs*2*sda+(nx+nu)%bs+(nx+pad+nu+jj)*bs];
	dsyrk_dpotrf_pp_lib(nz, nx, ((nu+2-1)/2)*2, hpL[0], 2*sda, hpQ[0], sda, diag);
	for(jj=0; jj<nu; jj++) hpL[0][(nx+pad)*bs+(jj/bs)*bs*(2*sda)+jj%bs+jj*bs] = diag[jj]; // copy reciprocal of diagonal
	...
	for(ii=0; ii<N; ii++)
		{
		for(jj=0; jj<nu; jj++) hux[ii][jj] = - hpL[ii][(nx+pad)*bs+((nu+nx)/bs)*bs*(2*sda)+(nu+nx)%bs+bs*jj];
...
/*		dtrsv_p_t_lib(nu, &hpL[ii][(nx+pad)*bs], 2*sda, &hux[ii][0]);*/
		dtrsv_dgemv_p_t_lib(nx+nu, nu, &hpL[ii][(nx+pad)*bs], 2*sda, &hux[ii][0]);
...
/*		dgemv_p_t_lib(nx, nu, nu, &hpL[ii][(nx+pad)*bs+(nu/bs)*bs*(2*sda)+nu%bs], 2*sda, &hux[ii][nu], &hux[ii][0], -1);*/
/*		dtrsv_p_t_lib(nu, &hpL[ii][(nx+pad)*bs], 2*sda, &hux[ii][0]);*/
/*d_print_pmat(nu, nu, bs, &hpL[ii][(nx+pad)*bs], 2*sda);*/
		dtrsv_dgemv_p_t_lib(nx+nu, nu, &hpL[ii][(nx+pad)*bs], 2*sda, &hux[ii][0]);
/*d_print_mat(1, nu, &hux[ii][0], 1);*/
/*if(ii==1)*/
/*exit(3);*/
		for(jj=0; jj<nx; jj++) hux[ii+1][nu+jj] = hpBAbt[ii][((nu+nx)/bs)*bs*sda+(nu+nx)%bs+bs*jj];
		dgemv_p_t_lib(nx+nu, nx, 0, hpBAbt[ii], sda, &hux[ii][0], &hux[ii+1][nu], 1);
