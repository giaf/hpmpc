...
void d_update_hessian_box(int k0, int kmax, int nb, int cnz, double sigma_mu, double *t, double *lam, double *lamt, double *dlam, double *bd, double *bl, double *pd, double *pl, double *db)
	...
	__m256d
		v_ones, v_sigma_mu,
		v_tmp, vlam, v_lamt, v_dlam;
		
	__m128d
		u_lamt, u_bd, u_lam, u_lb, u_ub;
...
/*		v_tmp  = _mm256_div_pd( ones, v_tmp );*/
/*		v_lam  = _mm256_load_pd( &lam[0] );*/
/*		v_lamt = _mm256_mul_pd( v_tmp, v_lam );*/
/*		_mm256_store_pd( v_lamt, &lamt[0] );*/
/*		v_dlam = _mm256_mul_pd( v_tmp, v_sigma_mu );*/
/*		_mm256_store_pd( v_dlam, &dlam[0] );*/
/*		u_lamt = _mm256_extractf128_pd( v_lamt, 0x1 );*/
/*		u_lamt = _mm_hadd_pd( u_lamt, _mm256_castpd256_pd128( u_lamt ) ); // [ lamt[0]+lamt[1] , lamt[2]+lamt[3] ]*/
...
/*		u_lam  = _mm_hsub_pd( u_lam, _mm256_castpd256_pd128( u_lam ) ); // [ lam[1]-lam[0] , lam[3]-lam[2] ] + [ dlam[1]-dlam[0] , dlam[3]-dlam[2] ]*/
/*		u_lb   = _mm_load_pd( &lb[ii] ); // [lb[0] , lb[1] ]*/
/*		u_ub   = _mm_load_pd( &ub[ii] ); // [ub[0] , ub[1] ]*/
/*		u_tmp0 = _mm_unpacklo_pd( u_lb, u_ub ); // [lb[0] , ub[0]]*/
/*		u_tmp1 = _mm_unpackhi_pd( u_lb, u_ub ); // [lb[1] , ub[1]]*/
/*		// TODO*/
