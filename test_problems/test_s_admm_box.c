...
************************************************/
	...
	float *work; s_zeros_align(&work, (N+1)*(pnz*cnl + 6*anz + 2*anx) + 3*anz, 1); // work space
...
/*	float mu = -1.0;*/
	...
	if(FREE_X0==0)
		{
		s_admm_box_mpc(&kk, k_max, tol, tol, warm_start, 1, rho, alpha, stat, nx, nu, N, hpBAbt, hpQ, hlb, hub, hux, compute_mult, hpi, work);
		}
	else
		{
/*		d_ip_box_mhe(&kk, k_max, tol, warm_start, sigma, stat, nx, nu, N, nb, hpBAbt, hpQ, hdb, hux, compute_mult, hpi, hlam, ht, work);*/
	...
	for(rep=0; rep<nrep; rep++)
		...
		if(FREE_X0==0)
			{
			s_admm_box_mpc(&kk, k_max, tol, tol, warm_start, 0, rho, alpha, stat, nx, nu, N, hpBAbt, hpQ, hlb, hub, hux, compute_mult, hpi, work);
			}
		else
			{
/*			d_ip_box_mhe(&kk, k_max, tol, warm_start, sigma, stat, nx, nu, N, nb, hpBAbt, hpQ, hdb, hux, compute_mult, hpi, hlam, ht, work);*/
