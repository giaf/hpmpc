...
int c_order_ip_hard_mpc_tv( int *kk, int k_max, double mu0, double mu_tol, char prec,
...
//printf("\n%d\n", ((size_t) ptr) & 63);
		...
		if(time_invariant) // TODO time-invariant work space ?????
			...
			if(ng>0)
				...
				if(jj<N)
					{
					d_cvt_mat2pmat(nu, ng, D, nu, 0, hpDCt[ii], cng);
					d_cvt_mat2pmat(nx, ng, C, nx, nu, hpDCt[ii]+nu/bs*cng*bs+nu%bs, cng);
					}
				}
			// last stage
			if(ngN>0)
				...
				d_cvt_mat2pmat(nx, ngN, Cf, nx, 0, hpDCt[N], cngN);
				}
			...
			if(jj<N)
				{
				d_cvt_tran_mat2pmat(nu, nu, R, nu, 0, hpQ[jj], cnux);
				d_cvt_mat2pmat(nx, nu, S, nx, nu, hpQ[jj]+nu/bs*cnux*bs+nu%bs, cnux);
				d_cvt_tran_mat2pmat(nx, nx, Q, nx, nu, hpQ[jj]+nu/bs*cnux*bs+nu%bs+nu*bs, cnux);
				for(ii=0; ii<nu; ii++)
			...
			for(ii=1; ii<N; ii++)
				{
				d_cvt_mat2pmat(nu, nx, B+ii*nu*nx, nu, 0, hpBAbt[ii], cnx);
				d_cvt_mat2pmat(nx, nx, A+ii*nx*nx, nx, nu, hpBAbt[ii]+nu/bs*cnx*bs+nu%bs, cnx);
				for (jj = 0; jj<nx; jj++)
...
/*return 1;*/
		...
		if(ngN>0)
			{
			for(ii=0; ii<pnu*cngN; ii++) hpDCt[N][ii] = 0.0; // make sure D is zero !!!!!
			d_cvt_mat2pmat(nx, ngN, Cf, nx, nu, hpDCt[N]+nu/bs*cngN*bs+nu%bs, cngN);
			}
...
int c_order_ip_soft_mpc( int k_max, double mu_tol, char prec,
...
//		printf("\nmatrix space allocated\n");
		...
		d_cvt_tran_mat2pmat(nx, nx, L0, nx, 0, hpLp[0]+(nx+nw+pad)*bs, cnj);
		...
		for(ii=0; ii<N; ii++)
			...
			d_cvt_tran_mat2pmat(nx, nx, A+ii*nx*nx, nx, 0, hpA[ii], cnx);
			d_cvt_tran_mat2pmat(nw, nx, G+ii*nx*nw, nw, 0, hpG[ii], cnw);
			d_cvt_tran_mat2pmat(ny, nx, C+ii*ny*nx, ny, 0, hpC[ii], cnx);
			for(jj=0; jj<nx; jj++) hf[ii][jj] = f[ii*nx+jj];
			// cost function
			d_cvt_tran_mat2pmat(nw, nw, Q+ii*nw*nw, nw, 0, hpQ[ii], cnw);
			d_cvt_tran_mat2pmat(ny, ny, R+ii*ny*ny, ny, 0, hpR[ii], cny);
		...
		// stage N
		// dynamic system
		d_cvt_tran_mat2pmat(nx, ny, C+N*ny*nx, nx, 0, hpC[N], cnx);
		// cost function
		d_cvt_tran_mat2pmat(ny, ny, R+N*ny*ny, ny, 0, hpR[N], cny);
		...
		d_cvt_tran_pmat2mat(nx, nx, 0, hpLp[1]+(nx+nw+pad)*bs, cnj, L0, nx);
		...
		d_cvt_tran_pmat2mat(nx, nx, ny, hpLe[N]+(ny/bs)*bs*cnf+ny%bs+ny*bs, cnf, Le, nx);
	...
	if(prec=='d')
		...
		for(ii=0; ii<N; ii++)
			...
			if(nx>=nw)
				...
				if(alg==2)
					{
					d_cvt_tran_mat2pmat(nx, nx, Q+ii*nx*nx, nx, 0, hpQRAG[ii], cnwx1);
					}
				else
					{
					...
					d_cvt_tran_mat2pmat(ny, ny, Q+ii*ny*ny, ny, 0, hpQRAG[ii], cnwx1);
					if(alg==0)
						{
						hpCt[ii] = ptr;
						ptr += pnx*cny;
				...
				d_cvt_tran_mat2pmat(nx, nx, A+ii*nx*nx, nx, 0, hpQRAG[ii]+pnx*cnwx1, cnwx1);
				d_cvt_tran_mat2pmat(nw, nw, R+ii*nw*nw, nw, 0, hpQRAG[ii]+(pnx-pnw)*cnwx1+nx*bs, cnwx1);
				d_cvt_tran_mat2pmat(nw, nx, G+ii*nw*nx, nw, 0, hpQRAG[ii]+pnx*cnwx1+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				if(nx>pnx-nx)
					d_cvt_tran_mat2pmat(nx, pnx-nx, A+ii*nx*nx+(nx-pnx+nx)*nx, nx, nx, hpQRAG[ii]+nx/bs*bs*cnwx1+nx%bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nx, nx, A+ii*nx*nx, nx, nx, hpQRAG[ii]+nx/bs*bs*cnwx1+nx%bs, cnwx1);
				if(nx>pnw-nw)
					d_cvt_tran_mat2pmat(nw, pnw-nw, G+ii*nw*nx+(nx-pnw+nw)*nw, nw, nw, hpQRAG[ii]+(pnx-pnw+nw/bs*bs)*cnwx1+nw%bs+nx*bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nw, nx, G+ii*nw*nx, nw, nw, hpQRAG[ii]+(pnx-pnw+nw/bs*bs)*cnwx1+nw%bs+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				}
			else
				{
				if(alg==2)
					{
					d_cvt_tran_mat2pmat(nx, nx, Q+ii*nx*nx, nx, 0, hpQRAG[ii]+(pnw-pnx)*cnwx1, cnwx1);
					}
				else
					{
					for(jj=0; jj<pnx; jj+=4) // loop on panels 
						for(ll=0; ll<nx*bs; ll++) // loop within panels
							hpQRAG[ii][ll+jj*bs*cnwx1] = 0.0;
					d_cvt_tran_mat2pmat(ny, ny, Q+ii*ny*ny, ny, 0, hpQRAG[ii]+(pnw-pnx)*cnwx1, cnwx1);
					if(alg==0)
						{
						hpCt[ii] = ptr;
						ptr += pnx*cny;
				...
				d_cvt_tran_mat2pmat(nx, nx, A+ii*nx*nx, nx, 0, hpQRAG[ii]+pnw*cnwx1, cnwx1);
				d_cvt_tran_mat2pmat(nw, nw, R+ii*nw*nw, nw, 0, hpQRAG[ii]+nx*bs, cnwx1);
				d_cvt_tran_mat2pmat(nw, nx, G+ii*nw*nx, nw, 0, hpQRAG[ii]+pnw*cnwx1+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				if(nx>pnx-nx)
					d_cvt_tran_mat2pmat(nx, pnx-nx, A+ii*nx*nx+(nx-pnx+nx)*nx, nx, nx, hpQRAG[ii]+(pnw-pnx+nx/bs*bs)*cnwx1+nx%bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nx, nx, A+ii*nx*nx, nx, nx, hpQRAG[ii]+(pnw-pnx+nx/bs*bs)*cnwx1+nx%bs, cnwx1);
				if(nx>pnw-nw)
					d_cvt_tran_mat2pmat(nw, pnw-nw, G+ii*nw*nx+(nx-pnw+nw)*nw, nw, nw, hpQRAG[ii]+nw/bs*bs*cnwx1+nw%bs+nx*bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nw, nx, G+ii*nw*nx, nw, nw, hpQRAG[ii]+nw/bs*bs*cnwx1+nw%bs+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				}
			}
		hpQRAG[N] = ptr;
		...
		if(alg==2)
			{
			d_cvt_tran_mat2pmat(nx, nx, Qf, nx, 0, hpQRAG[N], cnx);
			}
		else // if(alg==0 || alg==1)
			{
			for(jj=0; jj<pnx*cnx; jj++) hpQRAG[N][jj] = 0.0;
			d_cvt_tran_mat2pmat(ny, ny, Qf, ny, 0, hpQRAG[N], cnx);
			if(alg==0)
				{
				hpCt[N] = ptr;
				ptr += pnx*cny;
		...
		if(ndN>0)
			{
			d_cvt_tran_mat2pmat(nx, ndN, D, nx, 0, hpQRAG[N]+pnx*cnx, cnx);
			//d_print_pmat(pnx+pndN, cnx, bs, pQD, cnx);
			if(ndN>pnx-nx)
				d_cvt_tran_mat2pmat(nx, pnx-nx, D+(ndN-pnx+nx)*nx, nx, nx, hpQRAG[N]+nx/bs*bs*cnx+nx%bs, cnx);
			else
				d_cvt_tran_mat2pmat(nx, ndN, D, nx, nx, hpQRAG[N]+nx/bs*bs*cnx+nx%bs, cnx);
			//d_print_pmat(pnx+pndN, cnx, bs, pQD, cnx);
			Ld = ptr;
			ptr += pndN*cndN;
			}
		...
		d_cvt_tran_mat2pmat(nx, nx, L0, nx, 0, hpLe[0], cnx);
		...
		//dtrinv_lib(nx, pL0, cnx, pL0_inv2, cnx); // XXX
		d_cvt_tran_mat2pmat(nx, nx, L0, nx, 0, pL0_inv2, cnx); // XXX
		...
		d_cvt_tran_pmat2mat(nx, nx, 0, hpLe[1], cnx, L0, nx);
		...
		d_cvt_tran_pmat2mat(nx, nx, 0, hpLAG[N], cnx, Le, nx);
...
int c_order_admm_box_wrapper( int k_max, double tol,
...
/*		double alpha = 1.9; // relaxation parameter*/
        ...
        for(ii=0; ii<N; ii++)
	        {
            d_cvt_mat2pmat(nx, nu, B+ii*nu*nx, nx, 0, hpBAbt[ii], cnx);
            d_cvt_mat2pmat(nx, nx, A+ii*nx*nx, nx, nu, hpBAbt[ii]+nu/bs*cnx*bs+nu%bs, cnx);
            for (jj = 0; jj<nx; jj++)
                hpBAbt[ii][(nx+nu)/bs*cnx*bs+(nx+nu)%bs+jj*bs] = b[ii*nx+jj];
	        }
...
/*return 1;*/
        ...
        for(jj=0; jj<N; jj++)
            ...
            d_cvt_mat2pmat(nu, nx, S+jj*nx*nu, nu, nu, hpQ[jj]+nu/bs*cnz*bs+nu%bs, cnz);
