...
//	ptr += (N+1)*(pnx + pnz*cnl + 12*pnz) + 3*pnz;
	...
	*kk = 0;	
	...
	while( *kk<k_max && mu>mu_tol && alpha>=alpha_min )
		...
		s_update_hessian_box_mhe(N, nbu, (nu/bs)*bs, nb, cnz, 0.0, t, t_inv, lam, lamt, dlam, bd, bl, pd, pl, pl2, db);
		...
		// compute the affine duality gap
		s_compute_mu_mhe(N, nbu, nu, nb, &mu_aff, mu_scal, alpha, lam, dlam, t, dt);
		...
		// solve the system
		s_ric_trs_mhe(nx, nu, N, pBAbt, pL, pl2, dux, work, 1, Pb, compute_mult, dpi);
