...
//d_set_pmat(nz, nz, 0.0, 0, hpL[N], cnl);
	...
	if(ngN>0)
		{
		dgemv_n_lib(nu+nx, ngN, hpDCt[N], cngN, qx[N]+2*pnb, hQl[N], hQl[N], 1);
		dgemm_diag_right_lib(nu+nx, ngN, hpDCt[N], cngN, Qx[N]+2*pnb, pLBAbtDCt, cngN, pLBAbtDCt, cngN, 0);
...
//d_set_pmat(nz, nz, 0.0, 0, hpL[N], cnl);
	...
	if(ngN>0)
		{
		dgemm_diag_right_lib(nux, ngN, hpDCt[N], cngN, Qx[N]+2*pnb, pLBAbtDCt, cngN, pLBAbtDCt, cngN, 0);
		}
	if(update_hessian)
		{
	...
	for(nn=0; nn<N-1; nn++)
		...
		if(ng>0)
			{
			dgemm_diag_right_lib(nux, ng, hpDCt[N-nn-1], cng, Qx[N-nn-1]+2*pnb, pLBAbtDCt+nx*bs, cnxg, pLBAbtDCt+nx*bs, cnxg, 0);
			}
		if(update_hessian)
			{
	...
	if(fixed_x0==1) // mpc
		...
		if(ng>0)
			{
			dgemm_diag_right_lib(nux, ng, hpDCt[0], cng, Qx[0]+2*pnb, pLBAbtDCt+nx*bs, cnxg, pLBAbtDCt+nx*bs, cnxg, 0);
			}
		if(update_hessian)
			{
...
void d_back_ric_trs_new(int N, int nx, int nu, double **hpBAbt, double **hq, int fixed_x0, double **hux, double **hpL, double **hdL, double *work, int compute_Pb, double **hPb, int compute_pi, double **hpi, int nb, int ng, int ngN, double **hpDCt, double **qx)
	...
	for(nn=0; nn<N-1; nn++)
		...
		if(compute_Pb)
			...
			dtrmv_u_n_lib(nx, hpL[N-nn]+(ncl)*bs, cnl, hPb[N-nn-1], work, 0);
			dtrmv_u_t_lib(nx, hpL[N-nn]+(ncl)*bs, cnl, work, hPb[N-nn-1], 0); // L*(L'*b)
			}
		// general constraints
		if(ng>0)
			{
			dgemv_n_lib(nx+nu, ng, hpDCt[N-nn-1], cng, qx[N-nn-1]+2*pnb, hq[N-nn-1], hq[N-nn-1], 1);
			}
		for(jj=0; jj<nx; jj++) work[jj] = hPb[N-nn-1][jj] + hq[N-nn][nu+jj]; // add p
		dgemv_n_lib(nx+nu, nx, hpBAbt[N-nn-1], cnx, work, hq[N-nn-1], hq[N-nn-1], 1);
	...
	if(fixed_x0==1) // mpc
		...
		if(compute_Pb)
			...
			dtrmv_u_n_lib(nx, hpL[1]+(ncl)*bs, cnl, hPb[0], work, 0);
			dtrmv_u_t_lib(nx, hpL[1]+(ncl)*bs, cnl, work, hPb[0], 0); // L*(L'*b)
			}
		// general constraints
		if(ng>0)
			{
			dgemv_n_lib(nx+nu, ng, hpDCt[0], cng, qx[0]+2*pnb, hq[0], hq[0], 1);
			}
		for(jj=0; jj<nx; jj++) work[jj] = hPb[0][jj] + hq[1][nu+jj]; // add p
		dgemv_n_lib(nx+nu, nx, hpBAbt[0], cnx, work, hq[0], hq[0], 1);
