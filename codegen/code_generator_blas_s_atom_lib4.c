...
fprintf(f, "	kernel_spotrf_strsv_4x4_c99_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            	...
	            	for(; j<nna-3; j+=4)
	            		...
	            		for(; i<n; i+=4)     
	            			{     
fprintf(f, "	kernel_sgemm_pp_nt_4x4_atom_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
	            			}     
fprintf(f, "	kernel_spotrf_strsv_4x4_c99_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            		}     
		}
...
void strmm_ppp_code_generator(FILE *f, int m, int n, int offset)
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_sgemm_pp_nt_4x4_atom_lib4(%d, &pA[%d], &pB[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
...
/*	const int sdb = PNZ;*/
	...
	for(; i<m; i+=4)
		...
		for(; j<j_end; j+=4)
			{
fprintf(f, "	kernel_sgemm_pp_nt_4x4_atom_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
