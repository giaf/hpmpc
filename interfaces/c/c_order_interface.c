...
/*printf("\nstart of ip solver\n");*/
        ...
        // call the IP solver
	    if(IP==1)
	        hpmpc_status = d_ip_soft_mpc(nIt, k_max, mu0, mu_tol, alpha_min, warm_start, sigma_par, stat, nx, nu, N, nb, hpBAbt, hpQ, hZ, hz, hdb, hux, compute_mult, hpi, hlam, ht, work);
	    else
	        hpmpc_status = d_ip2_soft_mpc(nIt, k_max, mu0, mu_tol, alpha_min, warm_start, sigma_par, stat, nx, nu, N, nb, hpBAbt, hpQ, hZ, hz, hdb, hux, compute_mult, hpi, hlam, ht, work);
...
//		int compute_mult = 1; // compute multipliers
		...
		if(alg==2)
			...
			d_cvt_tran_mat2pmat(nx, nx, 0, bs, Qf, nx, hpQA[N], cnx);
			}
		else // if(alg==0 || alg==1)
			{
			for(jj=0; jj<pnx*cnx; jj++) hpQA[N][jj] = 0.0;
			//d_cvt_tran_mat2pmat(ny, ny, 0, bs, Q+N*ny*ny, ny, hpQA[N], cnx);
			d_cvt_tran_mat2pmat(ny, ny, 0, bs, Qf, ny, hpQA[N], cnx);
			}
		if(alg==0)
			{
			hpCt[N] = ptr;
