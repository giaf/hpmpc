...
void d_ric_sv_mpc_tv(int N, int *nx, int *nu, double **hpBAbt, double **hpQ, double **hux, double **hpL, double *work, double *diag, int compute_Pb, double **hPb, int compute_pi, double **hpi, int *nb, int **idxb, double **hQd, double **hQl, int *ng, double **hpDCt, double **Qx, double **qx, int fast_rsqrt)
	...
	d_print_pmat(nu0+nx0+1, nu0+nx0, bs, hpL[N], cnl0);
	exit(1);
#endif
	...
	for(nn=0; nn<N-1; nn++)
		...
		if(nz0<128)
			{
			dsyrk_dpotrf_lib(nz0, nu0+nx0, nx1+ng0, work, cnxg0, hpQ[N-nn-1], cnz0, hpL[N-nn-1], cnl0, diag, 1, fast_rsqrt);
			}
		else
			{
	...
	for(nn=0; nn<N; nn++)
		...
		if(compute_pi)
			...
			dtrmv_u_n_lib(nx1, hpL[nn+1]+(ncl)*bs, cnl1, &work[pnx1], &work[0], 1);
...
//d_print_mat(1, nu0+nx0, hl[N], 1);
		dgemv_n_lib(nu0+nx0, ng0, hpDCt[N], cng0, qx[N]+2*pnb0, hl[N], hl[N], 1);
//d_print_mat(1, nu0+nx0, hl[N], 1);
		}
	// middle stages
	for(nn=0; nn<N; nn++)
		...
		if(ng0>0)
			...
			cng0 = (ng0+ncl-1)/ncl*ncl;
			pnb0 = (nb0+bs-1)/bs*bs;
			dgemv_n_lib(nu0+nx0, ng0, hpDCt[N-nn-1], cng0, qx[N-nn-1]+2*pnb0, hl[N-nn-1], hl[N-nn-1], 1);
			}
		for(jj=0; jj<nx1; jj++) work[jj] = hPb[N-nn-1][jj] + hl[N-nn][nu1+jj]; // add p
		dgemv_n_lib(nx0+nu0, nx1, hpBAbt[N-nn-1], cnx1, work, hl[N-nn-1], hl[N-nn-1], 1);
		dtrsv_n_lib(nu0+nx0, nu0, 1, hpL[N-nn-1], cnl0, hl[N-nn-1]);
		}
	...
	for(nn=0; nn<N; nn++)
		...
		dtrsv_t_lib(nu0+nx0, nu0, 1, hpL[nn], cnl0, &hux[nn][0]);
		dgemv_t_lib(nu0+nx0, nx1, hpBAbt[nn], cnx1, &hux[nn][0], &hux[nn+1][nu1], &hux[nn+1][nu1], 1);
		if(compute_pi)
			{
			for(jj=0; jj<nx1; jj++) work[pnx1+jj] = hux[nn+1][nu1+jj]; // copy x into aligned memory
...
void d_back_ric_sv_new(int N, int nx, int nu, double **hpBAbt, double **hpQ, int update_hessian, double **hQd, double **hQl, int fixed_x0, double **hux, double **hpL, double **hl, double *pLBAbt, double *work, int compute_Pb, double **hPb, int compute_pi, double **hpi, int nb, int ng, int ngN, double **hpDCt, double **Qx, double **qx)
	...
	if(ngN>0)
		...
		dgemm_diag_right_lib(nx+nu, ngN, hpDCt[N], cngN, Qx[N]+2*pnb, pLBAbt, cngN, pLBAbt, cngN, 0);
		for(jj=0; jj<ngN; jj++)
			pLBAbt[(nu+nx)/bs*cngN*bs+(nu+nx)%bs+jj*bs] = 0.0;
		}
	if(update_hessian)
		{
	...
	dsyrk_dpotrf_lib_new(nx+nu%bs+1, nx+nu%bs, ngN, pLBAbt+(nu/bs)*bs*cngN, cngN, pLBAbt+(nu/bs)*bs*cngN, cngN, 1, hpQ[N]+(nu0/bs)*bs*cnz+(nu0/bs)*bs*bs, cnz, 0, dummy, hpL[N]+(nu0/bs)*bs*cnl+(nu0/bs)*bs*bs, cnl, hl[N]+nu);

#if 0
	d_print_pmat(nz, nx+ng, bs, pLBAbt, cnxg);
	...
	for(nn=0; nn<N-1; nn++)
		{	
		dtrmm_nt_u_lib(nz, nx, hpBAbt[N-nn-1], cnx, hpL[N-nn]+(ncl)*bs, cnl, pLBAbt, cnxg);
		if(compute_Pb)
			{
			for(jj=0; jj<nx; jj++) work[jj] = pLBAbt[((nx+nu)/bs)*bs*cnxg+(nx+nu)%bs+(jj)*bs];
			dtrmv_u_t_lib(nx, hpL[N-nn]+(ncl)*bs, cnl, work, hPb[N-nn-1], 0); // L*(L'*b)
			}
		for(jj=0; jj<nx; jj++) pLBAbt[((nx+nu)/bs)*bs*cnxg+(nx+nu)%bs+(jj)*bs] += hpL[N-nn][((nx+nu)/bs)*bs*cnl+(nx+nu)%bs+(nu+jj)*bs];
		if(ng>0)
			{
			dgemv_n_lib(nx+nu, ng, hpDCt[N-nn-1], cng, qx[N-nn-1]+2*pnb, hQl[N-nn-1], hQl[N-nn-1], 1);
			dgemm_diag_right_lib(nx+nu, ng, hpDCt[N-nn-1], cng, Qx[N-nn-1]+2*pnb, pLBAbt+nx*bs, cnxg, pLBAbt+nx*bs, cnxg, 0);
			for(jj=0; jj<ng; jj++)
				pLBAbt[(nu+nx)/bs*cnxg*bs+(nu+nx)%bs+(nx+jj)*bs] = 0.0;
			}
		if(update_hessian)
			{
		...
		dsyrk_dpotrf_lib_new(nz, nu+nx, nx+ng, pLBAbt, cnxg, pLBAbt, cnxg, 1, hpQ[N-nn-1], cnz, 0, dummy, hpL[N-nn-1], cnl, hl[N-nn-1]);
	...
	if(fixed_x0==1) // mpc
		...
		// first stage 
		dtrmm_nt_u_lib(nz, nx, hpBAbt[0], cnx, hpL[1]+(ncl)*bs, cnl, pLBAbt, cnxg);
		if(compute_Pb)
			{
			for(jj=0; jj<nx; jj++) work[jj] = pLBAbt[((nx+nu)/bs)*bs*cnxg+(nx+nu)%bs+(jj)*bs];
			dtrmv_u_t_lib(nx, hpL[1]+(ncl)*bs, cnl, work, hPb[0], 0); // L*(L'*b)
			}
		for(jj=0; jj<nx; jj++) pLBAbt[((nx+nu)/bs)*bs*cnxg+(nx+nu)%bs+(jj)*bs] += hpL[1][((nx+nu)/bs)*bs*cnl+(nx+nu)%bs+(nu+jj)*bs];
		if(ng>0)
			{
			dgemv_n_lib(nx+nu, ng, hpDCt[0], cng, qx[0]+2*pnb, hQl[0], hQl[0], 1);
			dgemm_diag_right_lib(nx+nu, ng, hpDCt[0], cng, Qx[0]+2*pnb, pLBAbt+nx*bs, cnxg, pLBAbt+nx*bs, cnxg, 0);
			for(jj=0; jj<ng; jj++)
				pLBAbt[(nu+nx)/bs*cnxg*bs+(nu+nx)%bs+(nx+jj)*bs] = 0.0;
			}
		if(update_hessian)
		...
		dsyrk_dpotrf_lib_new(nz, ((nu+2-1)/2)*2, nx+ng, pLBAbt, cnxg, pLBAbt, cnxg, 1, hpQ[0], cnz, 0, dummy, hpL[0], cnl, hl[0]);
		...
		dsyrk_dpotrf_lib_new(nz, nu+nx, nx+ng, pLBAbt, cnxg, pLBAbt, cnxg, 1, hpQ[0], cnz, 0, dummy, hpL[0], cnl, hl[0]);
...
void d_back_ric_sv(int N, int nx, int nu, double **hpBAbt, double **hpQ, int update_hessian, double **hQd, double **hQl, int fixed_x0, double **hux, double **hpL, double *work, double *diag, int compute_Pb, double **hPb, int compute_pi, double **hpi, int nb, int ng, int ngN, double **hpDCt, double **Qx, double **qx)
	...
	if(nx+nu%bs+1<128)
		{
		dsyrk_dpotrf_lib(nx+nu%bs+1, nx+nu%bs, ngN, work+(nu/bs)*bs*cngN, cngN, hpQ[N]+(nu/bs)*bs*cnz+(nu/bs)*bs*bs, cnz, hpL[N]+(nu/bs)*bs*cnl+(nu/bs)*bs*bs, cnl, diag, 1, 0);
		}
	else
		{
	...
	for(nn=0; nn<N-1; nn++)
		...
		if(compute_Pb)
			...
			dtrmv_u_t_lib(nx, hpL[N-nn]+(ncl)*bs, cnl, diag, hPb[N-nn-1], 0); // L*(L'*b)
			}
		...
		if(nz<128)
			{
			dsyrk_dpotrf_lib(nz, nu+nx, nx+ng, work, cnxg, hpQ[N-nn-1], cnz, hpL[N-nn-1], cnl, diag, 1, 0);
			}
		else
			{
	...
	if(fixed_x0==1) // mpc
		...
		if(compute_Pb)
			{
			for(jj=0; jj<nx; jj++) diag[jj] = work[((nx+nu)/bs)*bs*cnxg+(nx+nu)%bs+(jj)*bs];
			dtrmv_u_t_lib(nx, hpL[1]+(ncl)*bs, cnl, diag, hPb[0], 0); // L*(L'*b)
			}
		...
		dsyrk_dpotrf_lib(nz, ((nu+2-1)/2)*2, nx+ng, work, cnxg, hpQ[0], cnz, hpL[0], cnl, diag, 1, 0);
		for(ii=0; ii<nu; ii++) if(diag[ii]==0) diag[ii] = 1.0; // TODO needed ?????
		...
		nn = 0;
		for(jj=0; jj<nu; jj++) hux[0][jj] = - hpL[0][((nu+nx)/bs)*bs*cnl+(nu+nx)%bs+bs*jj];
		dtrsv_t_lib(nx+nu, nu, 1, hpL[0], cnl, &hux[0][0]);
		for(jj=0; jj<nx; jj++) hux[1][nu+jj] = hpBAbt[0][((nu+nx)/bs)*bs*cnx+(nu+nx)%bs+bs*jj];
		dgemv_t_lib(nx+nu, nx, hpBAbt[0], cnx, &hux[0][0], &hux[1][nu], &hux[1][nu], 1);
		if(compute_pi)
			{
			for(jj=0; jj<nx; jj++) work[anz+jj] = hux[1][nu+jj]; // copy x into aligned memory
			for(jj=0; jj<nx; jj++) work[jj] = hpL[1][((nu+nx)/bs)*bs*cnl+(nu+nx)%bs+bs*(nu+jj)]; // work space
			dtrmv_u_n_lib(nx, hpL[1]+(ncl)*bs, cnl, &work[anz], &work[0], 1);
			dtrmv_u_t_lib(nx, hpL[1]+(ncl)*bs, cnl, &work[0], &hpi[1][0], 0); // L*(L'*b) + p
			}
		...
		if(nz<128)
			{
			dsyrk_dpotrf_lib(nz, nu+nx, nx+ng, work, cnxg, hpQ[0], cnz, hpL[0], cnl, diag, 1, 0);
			}
		else
			{
			dsyrk_nt_lib(nz, nu+nx, nx+ng, work, cnxg, work, cnxg, hpQ[0], cnz, hpL[0], cnl, 1);
			dpotrf_lib(nz, nu+nx, hpL[0], cnl, hpL[0], cnl, diag);
			}

		for(ii=0; ii<nu; ii++) if(diag[ii]==0) diag[ii] = 1.0; // TODO needed ?????
		dtrtr_l_lib(nx, nu, hpL[0]+(nu/bs)*bs*cnl+nu%bs+nu*bs, cnl, hpL[0]+(ncl)*bs, cnl);	

		for(jj=0; jj<nu+nx; jj++) hpL[0][(jj/bs)*bs*cnl+jj%bs+jj*bs] = diag[jj]; // copy reciprocal of diagonal
		...
		drowex_lib(nu+nx, hpL[0]+(nx+nu)/bs*bs*cnl+(nu+nx)%bs, hux[0]);
		d_scale_mat(nu+nx, 1, -1.0, hux[0], 1);
		dtrsv_t_lib(nx+nu, nu+nx, 1, hpL[0], cnl, &hux[0][0]);
		for(jj=0; jj<nx; jj++) hux[nn+1][nu+jj] = hpBAbt[nn][((nu+nx)/bs)*bs*cnx+(nu+nx)%bs+bs*jj];
		dgemv_t_lib(nx+nu, nx, hpBAbt[nn], cnx, &hux[nn][0], &hux[nn+1][nu], &hux[nn+1][nu], 1);
		if(compute_pi)
			{
	...
	for(nn=1; nn<N; nn++)
		{
		for(jj=0; jj<nu; jj++) hux[nn][jj] = - hpL[nn][((nu+nx)/bs)*bs*cnl+(nu+nx)%bs+bs*jj];
		dtrsv_t_lib(nx+nu, nu, 1, hpL[nn], cnl, &hux[nn][0]);
		for(jj=0; jj<nx; jj++) hux[nn+1][nu+jj] = hpBAbt[nn][((nu+nx)/bs)*bs*cnx+(nu+nx)%bs+bs*jj];
		dgemv_t_lib(nx+nu, nx, hpBAbt[nn], cnx, &hux[nn][0], &hux[nn+1][nu], &hux[nn+1][nu], 1);
		if(compute_pi)
			{
...
void d_ric_trs_mpc(int nx, int nu, int N, double **hpBAbt, double **hpL, double **hq, double **hux, double *work, int compute_Pb, double ** hPb, int compute_pi, double **hpi, int nb, int ng, int ngN, double **hpDCt, double **qx)
	...
	if(ngN>0)
		{
		dgemv_n_lib(nx+nu, ngN, hpDCt[N], cngN, qx[N]+2*pnb, hq[N], hq[N], 1);
		}
	for(ii=0; ii<N; ii++)
		{
		if(compute_Pb)
		...
		if(ng>0)
			{
			dgemv_n_lib(nx+nu, ng, hpDCt[N-ii-1], cng, qx[N-ii-1]+2*pnb, hq[N-ii-1], hq[N-ii-1], 1);
			}
		for(jj=0; jj<nx; jj++) work[jj] = hPb[N-ii-1][jj] + hq[N-ii][nu+jj]; // add p
		dgemv_n_lib(nx+nu, nx, hpBAbt[N-ii-1], cnx, work, hq[N-ii-1], hq[N-ii-1], 1);
		dtrsv_n_lib(nu+nx, nu, 1, hpL[N-ii-1], cnl, hq[N-ii-1]);
		}
	...
	for(ii=0; ii<N; ii++)
		...
		dtrsv_t_lib(nx+nu, nu, 1, hpL[ii], cnl, &hux[ii][0]);
		dgemv_t_lib(nx+nu, nx, hpBAbt[ii], cnx, &hux[ii][0], &hux[ii+1][nu], &hux[ii+1][nu], 1);
		if(compute_pi)
			{
...
void d_ric_diag_trs_mpc(int N, int *nx, int *nu, double **hdA, double **hpBt, double **hpL, double **hpP, double **hb, double **hrq, double **hux, int compute_Pb, double **hPb, int compute_pi, double **hpi, double *work)
	...
	for(ii=0; ii<N; ii++)
		...
		if(compute_Pb)
			{
			dgemv_n_lib(nx1, nx1, hpP[N-ii], cnx1, hb[N-ii-1], hPb[N-ii-1], hPb[N-ii-1], 0);
			}
		for(jj=0; jj<nx1; jj++) work[jj] = hPb[N-ii-1][jj] + hpi[N-ii][jj];
		for(jj=0; jj<nu0; jj++) hux[N-ii-1][jj] = hrq[N-ii-1][jj];
		dgemv_n_lib(nu0, nx1, hpBt[N-ii-1], cnx1, work, hux[N-ii-1], hux[N-ii-1], 1);
		for(jj=0; jj<nxm; jj++) hpi[N-ii-1][jj] = hrq[N-ii-1][nu0+jj] + hdA[N-ii-1][jj] * work[jj];
		for(; jj<nx0; jj++) hpi[N-ii-1][jj] = hrq[N-ii-1][nu0+jj];
		dtrsv_n_lib(nu0, nu0, 1, hpL[N-ii-1], cnu0, hux[N-ii-1]);
		dgemv_n_lib(nx0, nu0, hpL[N-ii-1]+pnu0*cnu0, cnu0, hux[N-ii-1], hpi[N-ii-1], hpi[N-ii-1], -1);
		}
	...
	for(ii=0; ii<N; ii++)
		...
		dgemv_t_lib(nx0, nu0, hpL[ii]+pnu0*cnu0, cnu0, work, hux[ii], hux[ii], -1); // no overwrite of x
		dtrsv_t_lib(nu0, nu0, 1, hpL[ii], cnu0, hux[ii]); // no overwrite of x
		for(jj=0; jj<nx1; jj++) hux[ii+1][nu1+jj] = hb[ii][jj];
		dgemv_t_lib(nu0, nx1, hpBt[ii], cnx1, hux[ii], hux[ii+1]+nu1, hux[ii+1]+nu1, 1);
		for(jj=0; jj<nxm; jj++) 
			{
		...
		if(compute_pi)
			{
			dgemv_n_lib(nx1, nx1, hpP[ii+1], cnx1, work, hpi[ii+1], hpi[ii+1], 1); // P*b + p
			}
		}
...
void d_ric_trs_mhe_if(int nx, int nw, int ndN, int N, double **hpLe, double **hpLAG, double *Ld, double **hq, double **hr, double **hf, double **hxp, double **hx, double **hw, double **hlam, double *work)
	...
	for(ii=0; ii<N; ii++)
		...
		dtrmv_u_t_lib(nx, hpLe[ii], cnx, hxp[ii], x_temp, 0); // L*(L'*b) + p
		//d_print_mat(1, nx, x_temp, 1);
		dtrmv_u_n_lib(nx, hpLe[ii], cnx, x_temp, hx[ii], 1);
		//d_print_mat(1, nx, hx[ii], 1);
		...
		if(nx>pnx-nx)
			...
			dtrsv_n_lib(pnx, nx, 1, hpLAG[ii], cnwx1, hx[ii]);
			//d_print_mat(1, pnx, hx[ii], 1);
			//for(jj=0; jj<nx; jj++) hx[ii][jj] = - hx[ii][jj];
			//d_print_mat(1, nx, hx[ii], 1);
			for(jj=0; jj<nx-pnx+nx; jj++) hxp[ii+1][jj] = hf[ii][jj];
			dgemv_n_lib(nx-pnx+nx, nx, hpLAG[ii]+pnx*cnwx1, cnwx1, hx[ii], hxp[ii+1], hxp[ii+1], 1);
			...
			dtrsv_n_lib(nx+nx, nx, 1, hpLAG[ii], cnwx1, hx[ii]);
		...
		if(nx>pnw-nw)
			...
			dtrsv_n_lib(pnw, nw, 1, hpLAG[ii]+(pnx-pnw)*cnwx1+nx*bs, cnwx1, hw[ii]);
			//d_print_mat(1, pnw, hw[ii], 1);
			dgemv_n_lib(nx-pnw+nw, nw, hpLAG[ii]+pnx*cnwx1+nx*bs, cnwx1, hw[ii], hxp[ii+1], hxp[ii+1], -1);
			...
			dtrsv_n_lib(nw+nx, nw, 1, hpLAG[ii]+(pnx-pnw)*cnwx1+nx*bs, cnwx1, hw[ii]);
	...
	//d_print_mat(1, nx, hxp[N], 1);
	dtrmv_u_t_lib(nx, hpLe[N], cnx, hxp[N], x_temp, 0); // L*(L'*b) + p
	//d_print_mat(1, nx, x_temp, 1);
	dtrmv_u_n_lib(nx, hpLe[N], cnx, x_temp, hx[N], 1);
	//d_print_mat(1, nx, hx[N], 1);
	...
	if(ndN<=0)
		...
		//d_print_mat(1, nx, hx[N], 1);
		dtrsv_n_lib(nx, nx, 1, hpLAG[N], cnx, hx[N]);
		//d_print_mat(1, nx, hx[N], 1);
		dtrsv_t_lib(nx, nx, 1, hpLAG[N], cnx, hx[N]);
		//d_print_mat(1, nx, hx[N], 1);
		}
	else
		{
		...
		//d_print_pmat(ndN, ndN, bs, Ld, cndN);
		dtrsv_n_lib(ndN, ndN, 1, Ld, cndN, hlam[N]);
		//d_print_mat(1, ndN, hlam[N], 1);
		dtrsv_t_lib(ndN, ndN, 1, Ld, cndN, hlam[N]);
		//d_print_mat(1, ndN, hlam[N], 1);
		//exit(1);
		...
		if(ndN>pnx-nx)
			{
			//d_print_mat(1, pnx, hx[N], 1);
			dgemv_t_lib(ndN-pnx+nx, nx, hpLAG[N]+pnx*cnx, cnx, hlam[N], hx[N], hx[N], -1);
			//d_print_mat(1, pnx, hx[N], 1);
			for(jj=0; jj<pnx-nx; jj++) hx[N][nx+jj] = hlam[N][ndN-pnx+nx+jj];
			//d_print_mat(1, pnx, hx[N], 1);
			dtrsv_t_lib(pnx, nx, 1, hpLAG[N], cnx, hx[N]);
			}
		else
			{
			for(jj=0; jj<ndN; jj++) hx[N][nx+jj] = hlam[N][jj];
			dtrsv_t_lib(nx+ndN, nx, 1, hpLAG[N], cnx, hx[N]);
			}
...
//		dtrmv_u_n_lib(nx, hpLe[N-ii], cnx, x_temp+anx, hlam[N-ii-1], 0);
		...
		dtrmv_u_t_lib(nx, hpLe[N-ii], cnx, x_temp+pnx, x_temp, 0); // L*(L'*b) + p
		dtrmv_u_n_lib(nx, hpLe[N-ii], cnx, x_temp, x_temp+pnx, 0);
...
//			dtrsv_t_lib(pnx, nx, 1, hpLAG[N-ii-1], cnwx1, hx[N-ii-1]);
			...
			dtrsv_t_lib(2*nx, nx, 1, hpLAG[N-ii-1], cnwx1, x_temp);
			for(jj=0; jj<nx; jj++) hx[N-ii-1][jj] = x_temp[jj];
			//d_print_mat(1, nx, hx[N-ii-1], 1);
			}
		else
...
//			dtrsv_t_lib(nx+nx, nx, 1, hpLAG[N-ii-1], cnwx1, hx[N-ii-1]);
...
//			dtrsv_t_lib(pnw, nw, 1, hpLAG[N-ii-1]+(pnx-pnw)*cnwx1+nx*bs, cnwx1, hw[N-ii-1]);
			...
			//d_print_mat(1, pnx+nx, x_temp, 1);
			dtrsv_t_lib(nw+nx, nw, 1, hpLAG[N-ii-1]+(pnx-pnw)*cnwx1+nx*bs, cnwx1, x_temp+pnx-pnw);
			...
			dtrsv_t_lib(nw+nx, nw, 1, hpLAG[N-ii-1]+(pnx-pnw)*cnwx1+nx*bs, cnwx1, hw[N-ii-1]);
			}
	...
	for(ii=0; ii<N; ii++)
		...
		dtrsv_n_lib(2*nx, nx, 1, hpALe[ii]+cnx*bs, cnx2, x_temp);
		...
		dtrsv_n_lib(nw+nx, nw, 1, hpGLr[ii], cnw, x_temp);
	...
	if(ndN<=0)
		...
		//d_print_pmat(nx, nx, bs, hpALe[N]+cnx*bs, cnx2); 
		//d_print_mat(1, nx, hx[N], 1);
		dtrsv_n_lib(nx, nx, 1, hpALe[N]+cnx*bs, cnx2, hx[N]);
		//d_print_mat(1, nx, hx[N], 1);
		dtrsv_t_lib(nx, nx, 1, hpALe[N]+cnx*bs, cnx2, hx[N]);
		//d_print_mat(1, nx, hx[N], 1);
		}
	else
		{
		...
		dtrsv_n_lib(nx+ndN, nx, 1, hpALe[N]+cnx*bs, cnx2, x_temp);
		//d_print_mat(1, nx+ndN, x_temp, 1);
		//exit(1);
		...
		//d_print_mat(1, nx+ndN, x_temp, 1);
		dtrsv_t_lib(nx+ndN, nx, 1, hpALe[N]+cnx*bs, cnx2, x_temp);
	...
	for(ii=0; ii<N; ii++)
		...
		for(jj=0; jj<nx; jj++) x_temp[jj] = hx[N-ii-1][jj];
		for(jj=0; jj<nx; jj++) x_temp[nx+jj] = hlam[N-ii-1][jj];
		dtrsv_t_lib(nx+nx, nx, 1, hpALe[N-ii-1]+cnx*bs, cnx2, x_temp);
		for(jj=0; jj<nx; jj++) hx[N-ii-1][jj] = x_temp[jj];
		//d_print_mat(1, nx, hx[N-ii-1], 1);
		...
		dtrsv_t_lib(nw+nx, nw, 1, hpGLr[N-ii-1], cnw, x_temp);
...
int d_ric_trs_mhe(int nx, int nw, int ny, int N, double **hpA, double **hpG, double **hpC, double **hpLp, double **hdLp, double **hpQ, double **hpR, double **hpLe, double **hq, double **hr, double **hf, double **hxp, double **hxe, double **hw, double **hy, int smooth, double **hlam, double *work)
	...
	for(ii=0; ii<N; ii++)
		...
		// compute y + R*r
		dsymv_lib(ny, ny, hpR[ii], cny, hr[ii], y_temp, y_temp, -1);
		//d_print_mat(1, nz, y_temp, 1);
		...
		dgemv_n_lib(ny, nx, hpC[ii], cnx, hxp[ii], y_temp, y_temp, 1);
		//d_print_mat(1, nz, y_temp, 1);
		...
		// compute xe
		dtrsv_n_lib(ny+nx, ny, 1, hpLe[ii], cnf, y_temp);
		//d_print_mat(1, nz, y_temp, 1);
		...
		// xp += A*xe
		dgemv_n_lib(nx, nx, hpA[ii], cnx, hxe[ii], hxp[ii+1], hxp[ii+1], 1);
		//d_print_mat(1, nx, hxp[ii+1], 1);
		...
		// compute Q*q
		dsymv_lib(nw, nw, hpQ[ii], cnw, hq[ii], hw[ii], hw[ii], -1);
		//d_print_mat(1, nw, w_temp, 1);

		// xp += G*w
		dgemv_n_lib(nx, nw, hpG[ii], cnw, hw[ii], hxp[ii+1], hxp[ii+1], 1);
		//d_print_mat(1, nx, hxp[ii+1], 1);
	...
	// compute y + R*r
	dsymv_lib(ny, ny, hpR[N], cny, hr[N], y_temp, y_temp, -1);
	//d_print_mat(1, nz, y_temp, 1);

	// compute y + R*r - C*xp
	dgemv_n_lib(ny, nx, hpC[N], cnx, hxp[N], y_temp, y_temp, 1);
	//d_print_mat(1, nz, y_temp, 1);
	...
	// compute xe
	dtrsv_n_lib(ny+nx, ny, 1, hpLe[N], cnf, y_temp);
	//d_print_mat(1, nz, y_temp, 1);
	...
	for(ii=0; ii<N; ii++)
		...
		// else compute smooth values
			...
			// lam = \Pi^{-1}*lam
			dtrsv_n_lib(nx, nx, 1, hpLp[N-ii]+(nx+nw+pad)*bs, cnl, hlam[N-ii-1]);
			//d_print_mat(1, nx, hlam[N-ii-1], 1);
			dtrsv_t_lib(nx, nx, 1, hpLp[N-ii]+(nx+nw+pad)*bs, cnl, hlam[N-ii-1]);
			//d_print_mat(1, nx, hlam[N-ii-1], 1);
			...
			dgemv_t_lib(nx, nw, hpG[N-ii-1], cnw, hlam[N-ii-1], w_temp, w_temp, 0);
			//d_print_mat(nw, 1, w_temp, 1);

			// w = w - Q*G'*lam
			//d_print_pmat(nw, nw, bs, hpQ[N-ii-1], cnw);
			dsymv_lib(nw, nw, hpQ[N-ii-1], cnw, w_temp, hw[N-ii-1], hw[N-ii-1], -1);
			//d_print_mat(nw, 1, hw[N-ii-1], 1);

			// A'*lam
			//d_print_pmat(nx, nx, bs, hpA[N-ii-1], cnx);
			dgemv_t_lib(nx, nx, hpA[N-ii-1], cnx, hlam[N-ii-1], x_temp, x_temp, 0);
...
void d_ric_trs_mhe_end(int nx, int nw, int ny, int N, double **hpA, double **hpG, double **hpC, double **hpLp, double **hpQ, double **hpR, double **hpLe, double **hq, double **hr, double **hf, double **hxp, double **hxe, double **hy, double *work)
	...
	for(ii=0; ii<N; ii++)
		...
		// copy A*xp
		//for(jj=0; jj<nx; jj++) y_temp[ny+jj] = hxp[ii][jj];
		dgemv_n_lib(nx, nx, hpA[ii], cnx, hxp[ii], y_temp+ny, y_temp+ny, 0);
		//d_print_mat(1, nz, y_temp, 1);
		...
		// compute A*xe
		dtrsv_n_lib(ny+nx, ny, 1, hpLp[ii+1]+(nx+pad)*bs, cnl, y_temp);
		//d_print_mat(1, nz, y_temp, 1);
		...
		// compute Q*q
		dsymv_lib(nw, nw, hpQ[ii], cnw, hq[ii], w_temp, w_temp, -1);
		//d_print_mat(1, nw, w_temp, 1);

		// xp += G*w_temp
		dgemv_n_lib(nx, nw, hpG[ii], cnw, w_temp, hxp[ii+1], hxp[ii+1], 1);
		//d_print_mat(1, nx, hxp[ii+1], 1);
