...
fprintf(f, "	kernel_dpotrf_dtrsv_2x2_c99_lib2(%d, &pC[%d], %d);\n", n-j-2, j*bs+j*sdc, sdc);
	            	...
	            	for(; j<nna-1; j+=2)
	            		...
	            		for(; i<n; i+=2)     
	            			{     
fprintf(f, "	kernel_dgemm_pp_nt_2x2_c99_lib2(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
	            			}     
fprintf(f, "	kernel_dpotrf_dtrsv_2x2_c99_lib2(%d, &pC[%d], %d);\n", n-j-2, j*bs+j*sdc, sdc);
	            		}     
		}
...
void dtrmm_ppp_code_generator(FILE *f, int m, int n, int offset)
	...
	for(; i<m; i+=2)
		...
		for(; j<n-1; j+=2)
			{
fprintf(f, "	kernel_dgemm_pp_nt_2x2_c99_lib2(%d, &pA[%d], &pB[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
...
/*	const int sdb = PNZ;*/
	...
	for(; i<m; i+=2)
		...
		for(; j<j_end; j+=2)
			{
fprintf(f, "	kernel_dgemm_pp_nt_2x2_c99_lib2(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
