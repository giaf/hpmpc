...
/*	int n = m;*/
	...
	for(; j<n-2; j+=4)
		...
		if(i<m-8)
			{
			kernel_dsyrk_nt_12x4_lib4(k, &pA[i*sda], sda, &pB[j*sdb], &pD[j*bs+i*sdd], sdd, &pC[j*bs+i*sdc], sdc, alg);
			i += 12;
			for(; i<m-8; i+=12)
				{
				kernel_dgemm_nt_12x4_lib4(k, &pA[i*sda], sda, &pB[j*sdb], &pD[j*bs+i*sdd], sdd, &pC[j*bs+i*sdc], sdc, alg, 0, 0);
				}
			for(; i<m-4; i+=8)
				{
