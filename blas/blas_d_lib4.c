...
void dgemm_nt_lib(int m, int n, int k, double *pA, int sda, double *pB, int sdb, double *pC, int sdc, int alg)
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_m8x4_lib4(m-i, k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], alg);
			}
		jj = 0;
/*		for(; jj<n-j-1; jj+=2)*/
		for(; jj<n-j; jj+=2)
			{
			kernel_dgemm_nt_m8x2_lib4(m-i, k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], alg);
			}
/*		for(; jj<n-j; jj++)*/
/*			{*/
/*			kernel_dgemm_nt_8x1_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], alg);*/
/*			}*/
		}
	if(i<m)
		{
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_m4x4_lib4(m-i, k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
		jj = 0;
		for(; jj<n-j; jj+=2)
			{
			kernel_dgemm_nt_m4x2_lib4(m-i, k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
		}
#else
	...
	for(; i<m-8; i+=12)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_12x4_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pA[0+(i+8)*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], &pC[0+(j+0)*bs+(i+8)*sdc], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], &pC[0+(j+0)*bs+(i+8)*sdc], alg);
			}
		jj = 0;
/*		for(; jj<n-j-1; jj+=2)*/
		for(; jj<n-j; jj+=2)
			{
			kernel_dgemm_nt_8x2_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], alg);
			kernel_dgemm_nt_4x2_lib4(k, &pA[0+(i+8)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+(i+8)*sdc], &pC[0+(j+jj)*bs+(i+8)*sdc], alg);
			}
/*		for(; jj<n-j; jj++)*/
/*			{*/
/*			kernel_dgemm_nt_8x1_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], alg);*/
/*			}*/
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_8x4_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], alg);
			}
		jj = 0;
/*		for(; jj<n-j-1; jj+=2)*/
		for(; jj<n-j; jj+=2)
...
/*			}*/
	...
	for(; i<m-2; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_4x4_lib4(k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
		jj = 0;
/*		for(; jj<n-j-1; jj+=2)*/
		for(; jj<n-j; jj+=2)
			{
			kernel_dgemm_nt_4x2_lib4(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
/*		for(; jj<n-j; jj++)*/
/*			{*/
...
/*			}*/
	...
	for(; i<m; i+=2)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dgemm_nt_2x4_lib4(k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
		jj = 0;
/*		for(; jj<n-j-1; jj+=2)*/
		for(; jj<n-j; jj+=2)
			{
			kernel_dgemm_nt_2x2_lib4(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+0)*bs+i*sdc], alg);
			}
/*		for(; jj<n-j; jj++)*/
/*			{*/
/*			kernel_dgemm_nt_4x1_lib4(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], alg);*/
...
void dtrmm_l_lib(int m, int n, double *pA, int sda, double *pB, int sdb, double *pC, int sdc)
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
			kernel_dtrmm_l_nt_8x4_lib4(n-j-0, &pA[0+(j+0)*bs+i*sda], &pA[0+(j+0)*bs+(i+4)*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc]);
			}
		if(n-j==1)
			{
			corner_dtrmm_l_nt_8x1_lib4(&pA[0+(j+0)*bs+i*sda], &pA[0+(j+0)*bs+(i+4)*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc]);
			}
		else if(n-j==2)
			{
			corner_dtrmm_l_nt_8x2_lib4(&pA[0+(j+0)*bs+i*sda], &pA[0+(j+0)*bs+(i+4)*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc]);
			}
		else if(n-j==3)
			{
			corner_dtrmm_l_nt_8x3_lib4(&pA[0+(j+0)*bs+i*sda], &pA[0+(j+0)*bs+(i+4)*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc]);
			}
		}
#endif
	for(; i<m; i+=4)
...
/*	int n = m;*/
	...
	for(; j<n-2; j+=4)
		...
		if(i<m-4)
			{
#if defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2)
			kernel_dsyrk_dpotrf_nt_8x4_lib4(0, 0, j, &pC[i*sdc], &pC[(i+4)*sdc], &pC[j*sdc], &pD[j*bs+i*sdd], &pD[j*bs+(i+4)*sdd], &pC[j*bs+i*sdc], &pC[j*bs+(i+4)*sdc], fact, 1);
			i += 8;
#else
			kernel_dsyrk_dpotrf_nt_4x4_lib4(0, 0, j, &pC[i*sdc], &pC[j*sdc], &pD[j*bs+i*sdd], &pC[j*bs+i*sdc], fact, 1);
			i += 4;
#endif
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
	...
	for(; j<n; j+=2)
		...
		if(i<m-2)
			{
			kernel_dsyrk_dpotrf_nt_4x2_lib4(0, 0, j, &pC[i*sdc], &pC[j*sdc], &pD[j*bs+i*sdd], &pC[j*bs+i*sdc], fact, 1);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
			...
			{
			kernel_dsyrk_dpotrf_nt_2x2_lib4(0, 0, j, &pC[i*sdc], &pC[j*sdc], &pD[j*bs+i*sdd], &pC[j*bs+i*sdc], fact, 1);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
...
/*	int n = m;*/
	...
	for(; j<n-2; j+=4)
		...
		if(i<m-4)
			{
#if defined(TARGET_X64_AVX) || defined(TARGET_X64_AVX2)
			kernel_dsyrk_dpotrf_nt_8x4_lib4(0, k, j, &pA[i*sda], &pA[(i+4)*sda], &pA[j*sda], &pC[j*bs+i*sdc], &pC[j*bs+(i+4)*sdc], &pA[(k0+k+j)*bs+i*sda], &pA[(k0+k+j)*bs+(i+4)*sda], fact, alg);
			i += 8;
#else
			kernel_dsyrk_dpotrf_nt_4x4_lib4(0, k, j, &pA[i*sda], &pA[j*sda], &pC[j*bs+i*sdc], &pA[(k0+k+j)*bs+i*sda], fact, alg);
			i += 4;
#endif
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
	...
	for(; j<n; j+=2)
		...
		if(i<m-2)
			{
			kernel_dsyrk_dpotrf_nt_4x2_lib4(0, k, j, &pA[i*sda], &pA[j*sda], &pC[j*bs+i*sdc], &pA[(k0+k+j)*bs+i*sda], fact, alg);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
			...
			{
			kernel_dsyrk_dpotrf_nt_2x2_lib4(0, k, j, &pA[i*sda], &pA[j*sda], &pC[j*bs+i*sdc], &pA[(k0+k+j)*bs+i*sda], fact, alg);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
...
/*	int n = m;*/
	...
	for(; j<n-2; j+=4)
		...
		if(i<m-4)
			...
			for(; i<m-4; i+=8)
				{
				//d_print_pmat(8, k-i, bs, &pA[i*sda+i*bs], sda);
				kernel_dgemm_dtrsm_nt_8x4_lib4(0, 0, j, &pA[i*sda+(k+k0)*bs], &pA[(i+4)*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pC[j*bs+(i+4)*sdc], &pA[(k0+k+j)*bs+i*sda], &pA[(k0+k+j)*bs+(i+4)*sda], fact, alg);
				//d_print_pmat(8, 4, bs, &pA[(k0+k+j)*bs+i*sda], sda);
				}
#endif
			for(; i<m-2; i+=4)
				{
				kernel_dgemm_dtrsm_nt_4x4_lib4(0, 0, j, &pA[i*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pA[(k+k0+j)*bs+i*sda], fact, alg);
				//d_print_mat(4, 4, &pA[(k0+k+j)*bs+i*sda], 4);
				}
			for(; i<m; i+=2)
				{
				kernel_dgemm_dtrsm_nt_2x4_lib4(0, 0, j, &pA[i*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pA[(k+k0+j)*bs+i*sda], fact, alg);
				}
			}
		else //if(i<m-2)
			{
	...
	for(; j<n; j+=2)
		...
		if(i<m-2)
			{
			kernel_dsyrk_dpotrf_nt_4x2_lib4(1, k-i, j, &pA[i*sda+i*bs], &pA[i*sda+i*bs], &pC[j*bs+i*sdc], &pA[(k0+k+j)*bs+i*sda], fact, alg);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
			...
			for(; i<m-4 && i<k; i+=8)
				{
				kernel_dgemm_dtrsm_nt_8x2_lib4(1, k-i, j, &pA[i*sda+i*bs], &pA[(i+4)*sda+i*bs], &pA[j*sda+i*bs], &pC[j*bs+i*sdc], &pC[j*bs+(i+4)*sdc], &pA[(k0+k+j)*bs+i*sda], &pA[(k0+k+j)*bs+(i+4)*sda], fact, alg);
			...
			for(; i<m-4; i+=8)
				{
				kernel_dgemm_dtrsm_nt_8x2_lib4(1, 0, j, &pA[i*sda+(k+k0)*bs], &pA[(i+4)*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pC[j*bs+(i+4)*sdc], &pA[(k+k0+j)*bs+i*sda], &pA[(k+k0+j)*bs+(i+4)*sda], fact, alg);
				//d_print_pmat(8, 2, bs, &pA[(k0+k+j)*bs+i*sda], sda);
				}
#endif
			for(; i<m-2; i+=4)
				{
				kernel_dgemm_dtrsm_nt_4x2_lib4(0, 0, j, &pA[i*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pA[(k+k0+j)*bs+i*sda], fact, alg);
				//d_print_mat(4, 2, &pA[(k0+k+j)*bs+i*sda], 4);
				}
			for(; i<m; i+=2)
				{
				kernel_dgemm_dtrsm_nt_2x2_lib4(0, 0, j, &pA[i*sda+(k+k0)*bs], &pA[j*sda+(k+k0)*bs], &pC[j*bs+i*sdc], &pA[(k+k0+j)*bs+i*sda], fact, alg);
				}
			}
		else //if(i<m)
			{
			kernel_dsyrk_dpotrf_nt_2x2_lib4(1, k-i, j, &pA[i*sda+i*bs], &pA[i*sda+i*bs], &pC[j*bs+i*sdc], &pA[(k0+k+j)*bs+i*sda], fact, alg);
			diag[j+0] = fact[0];
			diag[j+1] = fact[2];
			if(fact[0]==0.0) fact[0]=1.0;
			if(fact[2]==0.0) fact[2]=1.0;
