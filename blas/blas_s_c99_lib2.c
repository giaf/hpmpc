...
#include "../include/kernel_s_c99_2x2.h"
...
void sgemm_ppp_nt_lib(int m, int n, int k, float *pA, int sda, float *pB, int sdb, float *pC, int sdc, int alg)
	...
	for(; i<m; i+=2)
		...
		for(; j<n-1; j+=2)
			{
			kernel_sgemm_pp_nt_2x2_c99_lib2(k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, alg);
			}
		jj = 0;
		for(; jj<n-j; jj++)
			{
			kernel_sgemm_pp_nt_2x1_c99_lib2(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], bs, alg);
			}
		}
...
void strmm_ppp_lib(int m, int n, int offset, float *pA, int sda, float *pB, int sdb, float *pC, int sdc)
	...
	for(; i<m; i+=2)
		...
		for(; j<n-1; j+=2)
			{
			kernel_sgemm_pp_nt_2x2_c99_lib2(n-j-0, &pA[0+(j+0)*bs+i*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, 0);
			}
		if(n-j==1)
			{
			corner_strmm_pp_nt_2x1_c99_lib2(&pA[0+(j+0)*bs+i*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs);
			}
		}
...
void ssyrk_ppp_lib(int m, int n, int k, float *pA, int sda, float *pC, int sdc)
	...
	for(; i<m; i+=2)
		...
		for(; j<j_end; j+=2)
			{
			kernel_sgemm_pp_nt_2x2_c99_lib2(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
			}
		if(n-j==1)
			{
			kernel_sgemm_pp_nt_2x1_c99_lib2(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
			}
		}
...
void spotrf_p_scopy_p_t_lib(int n, int nna, float *pC, int sdc, float *pL, int sdl)
	...
	if(j<nna-1)
		{
		kernel_spotrf_strsv_2x2_c99_lib2(n-j-2, &pC[0+j*bs+j*sdc], sdc);
		j += 2;
		for(; j<nna-1; j+=2)
			{
			i = j;
			for(; i<n; i+=2)
				{
				kernel_sgemm_pp_nt_2x2_c99_lib2(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
				}
			kernel_spotrf_strsv_2x2_c99_lib2(n-j-2, &pC[0+j*bs+j*sdc], sdc);
			}
		}
	...
	if(j==0) // assume that n>0
		{
		kernel_spotrf_strsv_scopy_2x2_c99_lib2(n-j-2, &pC[0+j*bs+j*sdc], sdc, (bs-nna%bs)%bs, &pL[0+(j-j0)*bs+((j-j0)/bs)*bs*sdc], sdl);
		j += 2;
		}
	for(; j<n-1; j+=2)
		{
	...
	if(n-j==1)
		{
		i = j;
		kernel_sgemm_pp_nt_2x1_c99_lib2(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
		corner_spotrf_strsv_scopy_1x1_c99_lib2(&pC[0+j*bs+j*sdc], sdc, (bs-nna%bs)%bs, &pL[0+(j-j0)*bs+((j-j0)/bs)*bs*sdc], sdl);
		}
...
void spotrf_p_lib(int m, int n, float *pC, int sdc)
	...
	for(; j<n-1; j+=2)
		...
		kernel_spotrf_strsv_2x2_c99_lib2(m-j-2, &pC[0+j*bs+j*sdc], sdc);
		}
	if(n-j==1)
		{
		i = j;
		...
		kernel_spotrf_strsv_1x1_c99_lib2(m-j-1, &pC[0+j*bs+j*sdc], sdc);
		}
...
void sgemv_p_n_lib(int n, int m, int offset, float *pA, int sda, float *x, float *y, int alg)
	...
	if(nna==1)
		{
		kernel_sgemv_n_1_c99_lib2(m, pA, x, y, alg);
		pA += 1;
		y  += 1;
		j++;
		pA += (sda-1)*bs;
...
/*	for(; j<n-3; j+=4)*/
/*		{*/
/*		kernel_sgemv_n_4_c99_lib2(m, pA, x, y, alg);*/
/*		pA += 2*sda*bs;*/
/*		y  += 2*bs;*/
/*		}*/
	for(; j<n-1; j+=2)
		{
		kernel_sgemv_n_2_c99_lib2(m, pA, x, y, alg);
		pA += sda*bs;
		y  += bs;
		}
	for(; j<n; j++)
...
/*	for(; j<m-3; j+=4)*/
/*		{*/
/*		kernel_sgemv_t_4_c99_lib2(n, nna, pA+j*bs, sda, x, y+j, alg);*/
/*		}*/
	for(; j<m-1; j+=2)
		{
		kernel_sgemv_t_2_c99_lib2(n, nna, pA+j*bs, sda, x, y+j, alg);
		}
	for(; j<m; j++)
		{
		kernel_sgemv_t_1_c99_lib2(n, nna, pA+j*bs, sda, x, y+j, alg);
		}
...
void strmv_p_n_lib(int m, int offset, float *pA, int sda, float *x, float *y)
	...
	if(mna==1)
		{
		kernel_sgemv_n_1_c99_lib2(j+1, pA, x, y, 1);
		pA += 1;
		y  += 1;
		pA += (sda-1)*bs;
		}
/*	for(; j<m-3; j+=4)*/
/*		{*/
/*		kernel_sgemv_n_4_c99_lib2(j+1, pA, x, y, 1);*/
...
/*		}*/
	for(; j<m-1; j+=2)
		{
		kernel_sgemv_n_2_c99_lib2(j+1, pA, x, y, 1);
...
void strmv_p_t_lib(int m, int offset, float *pA, int sda, float *x, float *y)
	...
	if(mna==1)
		{
		kernel_sgemv_t_1_c99_lib2(mmax-j, mna-j, pA+j*bs+j, sda, x+j, y+j, 1);
		pA += 1 + sda*bs;
		x  += 1;
		y  += 1;
		mmax -= 1;
	...
	for(; j<m-1; j+=2)
		{
		y[0] += pA[0+bs*0] * x[0];
		kernel_sgemv_t_2_c99_lib2(mmax, mmax, pA, sda, x, y, 1);
		pA += bs*sda + bs*bs;
		x  += bs;
		y  += bs;
		mmax -= bs;
	...
	for(; j<m; j++)
		{
		kernel_sgemv_t_1_c99_lib2(mmax, mmax, pA, sda, x, y, 1);
		pA += 1 + bs;
		x  += 1;
		y  += 1;
		mmax -= 1;
...
/*		}*/
	for(; j<m-1; j+=2)
		{
		kernel_sgemv_n_2_c99_lib2(j, pA, x, y, 1);
		ptrA = pA + j*bs;
		ptrx =  x + j;
		y[0] += ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1];
		y[1] += ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1];
		kernel_sgemv_t_2_c99_lib2(m-j-2, 0, ptrA+2, sda, ptrx+2, y, 1);
		pA += sda*bs;
		y  += bs;
		}
	for(; j<m; j++)
		{
		kernel_sgemv_n_1_c99_lib2(j, pA, x, y, 1);
		ptrA = pA + j*bs;
		ptrx =  x + j;
		y[0] += ptrA[0+bs*0]*ptrx[0];
		kernel_sgemv_t_1_c99_lib2(m-j-1, 0, ptrA+1, sda, ptrx+1, y, 1);
		pA += 1;
		y  += 1;
		}
...
/*		{*/
/*		// correct*/
/*		kernel_sgemv_n_4_c99_lib2(j, ptrA, x, ptrx, -1);*/
...
/*		}*/
	for(; j<n-1; j+=2)
		{
		// correct
		kernel_sgemv_n_2_c99_lib2(j, ptrA, x, ptrx, -1);
	...
	for(; j<n; j++)
		{
		// correct
		kernel_sgemv_n_1_c99_lib2(j, ptrA, x, ptrx, -1);
...
void strsv_p_t_lib(int n, float *pA, int sda, float *x)
	...
	if(rn==1)
		{
		i = rn-1-j;
		kernel_sgemv_t_1_c99_lib2(j, j, &ptrA[i+1+bs*(i+0)], sda, &ptrx[i+1], &ptrx[i], -1);
	...
	for(; j<qn; j++)
		...
		kernel_sgemv_t_2_c99_lib2(rn+j*bs, 0, ptrA+bs*sda, sda, ptrx+2, ptrx, -1);
