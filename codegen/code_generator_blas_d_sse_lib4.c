...
void dpotrf_p_dcopy_p_t_code_generator(FILE *f, int n, int nna)
	...
	if(j<nna-3)
		{
fprintf(f, "	kernel_dpotrf_dtrsv_4x4_sse_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            	j += 4;     
	            	for(; j<nna-3; j+=4)
	            		{     
	            		i = j;     
	            		for(; i<n; i+=4)     
	            			{     
fprintf(f, "	kernel_dgemm_pp_nt_4x4_sse_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
	            			}     
fprintf(f, "	kernel_dpotrf_dtrsv_4x4_sse_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            		}     
		}
...
fprintf(f, "	kernel_dpotrf_dtrsv_dcopy_4x4_sse_lib4(%d, &pC[%d], %d, %d, &pL[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
	...
	if(j<n)
		...
		if(n-j==1)
			{
			i = j;
fprintf(f, "	kernel_dgemm_pp_nt_4x1_sse_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
fprintf(f, "	corner_dpotrf_dtrsv_dcopy_1x1_sse_lib4(&pC[%d], %d, %d, &pL[%d], %d);\n", j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
			}
		else if(n-j==2)
			{
			i = j;
fprintf(f, "	kernel_dgemm_pp_nt_4x2_sse_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
fprintf(f, "	corner_dpotrf_dtrsv_dcopy_2x2_sse_lib4(&pC[%d], %d, %d, &pL[%d], %d);\n", j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
			}
		else if(n-j==3)
			{
			i = j;
fprintf(f, "	kernel_dgemm_pp_nt_4x3_sse_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
fprintf(f, "	corner_dpotrf_dtrsv_dcopy_3x3_sse_lib4(&pC[%d], %d, %d, &pL[%d], %d);\n", j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
			}
		}
...
fprintf(f, "	kernel_dpotrf_dtrsv_1x1_sse_lib4(%d, &pC[%d], %d);\n", m-j-1, j*bs+j*sdc, sdc);
			}
		else if(n-j==2)
			{
			i = j;
...
fprintf(f, "	kernel_dpotrf_dtrsv_2x2_sse_lib4(%d, &pC[%d], %d);\n", m-j-2, j*bs+j*sdc, sdc);
			}
		else if(n-j==3)
			{
			i = j;
...
fprintf(f, "	kernel_dpotrf_dtrsv_3x3_sse_lib4(%d, &pC[%d], %d);\n", m-j-3, j*bs+j*sdc, sdc);
			}
		}
...
void dtrmm_ppp_code_generator(FILE *f, int m, int n, int offset)
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_dgemm_pp_nt_4x4_sse_lib4(%d, &pA[%d], &pB[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x1_sse_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		else if(n-j==2)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x2_sse_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		else if(n-j==3)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x3_sse_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		}
...
/*	const int sdb = PNZ;*/
	...
	for(; i<m; i+=4)
		...
		if(j_end>n)
			...
			for(; j<n-3; j+=4)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x4_sse_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
				}
			if(n-j==1)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x1_sse_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
				}
			else if(n-j==2)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x2_sse_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
				}
			else if(n-j==3)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x3_sse_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
				}
			}
		else
			{
...
void dgemv_p_t_code_generator(FILE *f, int n, int m, int offset, int alg)
	...
	for(; j<m-7; j+=8)
		{
	fprintf(f, "	kernel_dgemv_t_8_sse_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m-3; j+=4)
		{
	fprintf(f, "	kernel_dgemv_t_4_sse_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m-1; j+=2)
		{
	fprintf(f, "	kernel_dgemv_t_2_sse_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m; j++)
		{
	fprintf(f, "	kernel_dgemv_t_1_sse_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
...
/*	fprintf(f, "	double *ptrA, *ptrx;\n");*/
	...
	for(; j<rn%2; j++)
		{
		i = rn-1-j;
	fprintf(f, "	kernel_dgemv_t_1_sse_lib4(%d, %d, &ptrA[%d], %d, &ptrx[%d], &ptrx[%d], -1);\n", j, j, i+1+bs*(i+0), sda, i+1, i);
	fprintf(f, "	ptrx[%d] = (ptrx[%d]) / ptrA[%d];\n", i+0, i+0, i+0+bs*(i+0));
		}
	for(; j<rn; j+=2)
		{
		i = rn-2-j;
	fprintf(f, "	kernel_dgemv_t_2_sse_lib4(%d, %d, &ptrA[%d], %d, &ptrx[%d], &ptrx[%d], -1);\n", j, j, i+2+bs*(i+0), sda, i+2, i);
	fprintf(f, "	ptrx[%d] = (ptrx[%d]) / ptrA[%d];\n", i+1, i+1, (i+1)+bs*(i+1));
	...
	fprintf(f, "	ptrx = x  + %d;\n", (qn-j-2)*bs);
		...
		// correct
	fprintf(f, "	kernel_dgemv_t_8_sse_lib4(%d, 0, ptrA+%d, sda, ptrx+8, ptrx, -1);\n", rn+j*bs, 2*bs*sda);
	...
	fprintf(f, "	ptrx = x  + %d;\n", (qn-j-2)*bs);
		...
		// correct
	fprintf(f, "	kernel_dgemv_t_4_sse_lib4(4, 0, ptrA+%d, sda, ptrx+4, ptrx, -1);\n", bs*sda);
	...
	fprintf(f, "	kernel_dgemv_t_4_sse_lib4(%d, 0, ptrA+%d, sda, ptrx+4, ptrx, -1);\n", rn+j*bs, bs*sda);
	fprintf(f, "	ptrx[3] = (ptrx[3]) / ptrA[%d];\n", 3+bs*3);
	fprintf(f, "	ptrx[2] = (ptrx[2] - ptrA[%d]*ptrx[3]) / ptrA[%d];\n", 3+bs*2, 2+bs*2);
	fprintf(f, "	ptrx[1] = (ptrx[1] - ptrA[%d]*ptrx[3] - ptrA[%d]*ptrx[2]) / ptrA[%d];\n", 3+bs*1, 2+bs*1, 1+bs*1);
	fprintf(f, "	ptrx[0] = (ptrx[0] - ptrA[%d]*ptrx[3] - ptrA[%d]*ptrx[2] - ptrA[%d]*ptrx[1]) / ptrA[%d];\n", 3+bs*0, 2+bs*0, 1+bs*0, 0+bs*0);
