...
void spotrf_p_scopy_p_t_code_generator(FILE *f, int n, int nna)
	...
	if(j<nna-1)
		{
fprintf(f, "	kernel_spotrf_strsv_2x2_lib2(%d, &pC[%d], %d, info);\n", n-j-2, j*bs+j*sdc, sdc);
...
fprintf(f, "	if(*info!=0) return;\n");
	...
	if(j==0) // assume that n>0
		{
fprintf(f, "	kernel_spotrf_strsv_scopy_2x2_lib2(%d, &pC[%d], %d, %d, &pL[%d], %d, info);\n", n-j-2, j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
fprintf(f, "	if(*info!=0) return;\n");
		j += 2;
		}
	for(; j<n-1; j+=2)
...
fprintf(f, "	corner_spotrf_strsv_scopy_1x1_lib2(&pC[%d], %d, %d, &pL[%d], %d, info);\n", j*bs+j*sdc, sdc, (bs-nna%bs)%bs, (j-j0)*bs+((j-j0)/bs)*bs*sdc, sdl);
fprintf(f, "	if(*info!=0) return;\n");
		}
...
fprintf(f, "	kernel_spotrf_strsv_1x1_lib2(%d, &pC[%d], %d, info);\n", m-j-1, j*bs+j*sdc, sdc);
fprintf(f, "	if(*info!=0) return;\n");
		}
...
void sgemv_p_n_code_generator(FILE *f, int n, int m, int offset, int alg)
	...
	if(nna==1)
		{
fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], x, &y[%d], %d);\n", m, idxA, idxy, alg);
		idxA += 1 + (sda-1)*bs;
		idxy += 1;
		j++;
		}
	for(; j<n-1; j+=2)
		{
fprintf(f, "	kernel_sgemv_n_2_lib2(%d, &pA[%d], x, &y[%d], %d);\n", m, idxA, idxy, alg);
		idxA += sda*bs;
		idxy += bs;
		}
	for(; j<n; j++)
...
fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], x, &y[%d], %d);\n", j+1, idxA, idxy, alg);
		...
		if(mna==1)
				{
fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], x, &y[%d], -1);\n", j+1, idxA, idxy);
			idxA += 1 + (sda-1)*bs;
			idxy += 1;
			j += 1;
			}
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_sgemv_n_2_lib2(%d, &pA[%d], x, &y[%d], -1);\n", j+1, idxA, idxy);
fprintf(f, "	y[%d] -= pA[%d] * x[%d];\n", idxy+1, idxA+(j+1)*bs+1, j+1);
			idxA += sda*bs;
			idxy += bs;
			}
...
void strmv_p_t_code_generator(FILE *f, int m, int offset, int alg)
	...
	if(alg==0 || alg==1)
		...
		if(mna==1)
			{
fprintf(f, "	kernel_sgemv_t_1_lib2(%d, 1, &pA[%d], %d, &x[%d], &y[%d], %d);\n", mmax, idxA, sda, idxx, idxy, alg);
			idxA += 1 + sda*bs;
			idxx += 1;
			idxy += 1;
			mmax -= 1;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_sgemv_t_2_lib2(%d, 1, &pA[%d], %d, &x[%d], &y[%d], %d);\n", mmax-1, idxA+1, sda, idxx+1, idxy, alg);
fprintf(f, "	y[%d] += pA[%d] * x[%d];\n", idxy, idxA, idxx);
			idxA += bs*sda + bs*bs;
			idxx += bs;
			idxy += bs;
		...
		for(; j<m; j++)
			{
fprintf(f, "	kernel_sgemv_t_1_lib2(%d, %d, &pA[%d], %d, &x[%d], &y[%d], %d);\n", mmax, mmax, idxA, sda, idxx, idxy, alg);
			idxA += 1 + bs;
			idxx += 1;
			idxy += 1;
			mmax -= 1;
		...
		if(mna==1)
			{
fprintf(f, "	kernel_sgemv_t_1_lib2(%d, 1, &pA[%d], %d, &x[%d], &y[%d], -1);\n", mmax, idxA, sda, idxx, idxy);
			idxA += 1 + sda*bs;
			idxx += 1;
			idxy += 1;
			mmax -= 1;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_sgemv_t_2_lib2(%d, 1, &pA[%d], %d, &x[%d], &y[%d], -1);\n", mmax-1, idxA+1, sda, idxx+1, idxy);
fprintf(f, "	y[%d] -= pA[%d] * x[%d];\n", idxy, idxA, idxx);
			idxA += bs*sda + bs*bs;
			idxx += bs;
			idxy += bs;
		...
		for(; j<m; j++)
			{
fprintf(f, "	kernel_sgemv_t_1_lib2(%d, %d, &pA[%d], %d, &x[%d], &y[%d], -1);\n", mmax, mmax, idxA, sda, idxx, idxy);
			idxA += 1 + bs;
			idxx += 1;
			idxy += 1;
			mmax -= 1;
...
/*		if(mna==1)*/
/*			{*/
/*fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], &x[%d], &y[%d], %d);\n", j, idxA, idxx, idxy, alg);*/
/*fprintf(f, "	y[%d] += pA[%d]*x[%d];\n", idxy, idxA+j*bs, idxx+j);*/
/*fprintf(f, "	kernel_sgemv_t_1_lib2(%d, 0, &pA[%d], %d, &x[%d], &y[%d], 1);\n", m-j-1, idxA+j*bs+1, sda, idxx+j+1, idxy);*/
/*			idxA += 1 + (sda-1)*bs;*/
/*			idxy += 1;*/
/*			j += 1;*/
/*			}*/
...
/*		if(mna==1)*/
/*			{*/
/*fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], &x[%d], &y[%d], -1);\n", j, idxA, idxx, idxy);*/
/*fprintf(f, "	y[%d] -= pA[%d]*x[%d];\n", idxy, idxA+j*bs, idxx+j);*/
/*fprintf(f, "	kernel_sgemv_t_1_lib2(%d, 0, &pA[%d], %d, &x[%d], &y[%d], -1);\n", m-j-1, idxA+j*bs+1, sda, idxx+j+1, idxy);*/
/*			idxA += 1 + (sda-1)*bs;*/
/*			idxy += 1;*/
...
fprintf(f, "	\n");
	...
	if(alg==0 || alg==1)
		...
		if(mna==1)
			{
fprintf(f, "	kernel_ssymv_1_lib2(%d, &pA[%d], &x[%d], &y[%d], &x[%d], &y[%d], 1, 1);\n", j, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += 1;
			idxy_n += 1;
			idxx_t += 1;
			idxA  += (sda-1)*bs;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_ssymv_2_lib2(%d, &pA[%d], &x[%d], &y[%d], &x[%d], &y[%d], 1, 1);\n", j, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += sda*bs;
			idxy_n += bs;
			idxx_t += bs;
			}
...
fprintf(f, "	\n");
	...
	if(alg==0 || alg==1)
		...
		if(mna==1)
			{
fprintf(f, "	kernel_ssymv_1_lib2(%d, &pA[%d], &x_n[%d], &y_n[%d], &x_t[%d], &y_t[%d], 0, 1);\n", n, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += 1;
			idxy_n += 1;
			idxx_t += 1;
			idxA  += (sda-1)*bs;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_ssymv_2_lib2(%d, &pA[%d], &x_n[%d], &y_n[%d], &x_t[%d], &y_t[%d], 0, 1);\n", n, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += sda*bs;
			idxy_n += bs;
			idxx_t += bs;
			}
...
/*	ptrx  = x;*/
	...
	for(; j<n-1; j+=2)
		{
		// correct
fprintf(f, "	kernel_sgemv_n_2_lib2(%d, &pA[%d], &x[0], &x[%d], -1);\n", j, idxA, idxx);
...
fprintf(f, "	x[%d] = (x[%d] - x[%d] * pA[%d]) / pA[%d];\n", idxx+1, idxx+1, idxx, idxAd+1, idxAd+1+bs*1);
	...
	for(; j<n; j++)
		{
		// correct
fprintf(f, "	kernel_sgemv_n_1_lib2(%d, &pA[%d], &x[0], &x[%d], -1);\n", j, idxA, idxx);
...
void strsv_p_t_code_generator(FILE *f, int n)
	...
	if(rn==1)
		{
		i = rn-1-j;
fprintf(f, "	kernel_sgemv_t_1_lib2(%d, %d, &pA[%d], %d, &x[%d], &x[%d], -1);\n", j, j, i+1+bs*i+qn*bs*(sda+bs), sda, i+1+qn*bs, i+qn*bs);
fprintf(f, "	x[%d] = (x[%d]) / pA[%d];\n", i+qn*bs, i+qn*bs, i+bs*i+qn*bs*(sda+bs) );
	...
	for(; j<qn; j++)
		...
		// first 2 rows
fprintf(f, "	kernel_sgemv_t_2_lib2(%d, 0, pA+%d, sda, x+%d+2, x+%d, -1);\n", rn+j*bs, bs*sda+(qn-j-1)*bs*(sda+bs), (qn-j-1)*bs, (qn-j-1)*bs);
