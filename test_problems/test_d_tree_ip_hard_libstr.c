...
#if KEEP_X0
	double  *pRSQrq0; d_zeros_align(&pRSQrq0, pnz, cnux);
	d_cvt_mat2pmat(nu, nu, R, nu, 0, pRSQrq0, cnux);
	d_cvt_tran_mat2pmat(nu, nx, S, nu, nu, pRSQrq0+nu/bs*bs*cnux+nu%bs, cnux);
	d_cvt_tran_mat2pmat(nu, 1, r, nu, nu+nx, pRSQrq0+(nu+nx)/bs*bs*cnux+(nu+nx)%bs, cnux);
	d_cvt_mat2pmat(nx, nx, Q, nx, nu, pRSQrq0+nu/bs*bs*cnux+nu%bs+nu*bs, cnux);
	d_cvt_tran_mat2pmat(nx, 1, q, nx, nu+nx, pRSQrq0+(nu+nx)/bs*bs*cnux+(nu+nx)%bs+nu*bs, cnux);
//	d_print_pmat(nu+nx+1, nu+nx, bs, pRSQrq0, cnux);
	...
	d_cvt_mat2pmat(nu, nu, R, nu, 0, pRSQrq0, cnu);
	d_cvt_tran_mat2pmat(nu, 1, r, nu, nu, pRSQrq0+nu/bs*bs*cnu+nu%bs, cnu);
//	d_print_pmat(nu+1, nu, bs, pRSQrq0, cnu);
	...
	d_cvt_mat2pmat(nu, nu, R, nu, 0, pRSQrq1, cnux);
	d_cvt_tran_mat2pmat(nu, nx, S, nu, nu, pRSQrq1+nu/bs*bs*cnux+nu%bs, cnux);
	d_cvt_tran_mat2pmat(nu, 1, r, nu, nu+nx, pRSQrq1+(nu+nx)/bs*bs*cnux+(nu+nx)%bs, cnux);
	d_cvt_mat2pmat(nx, nx, Q, nx, nu, pRSQrq1+nu/bs*bs*cnux+nu%bs+nu*bs, cnux);
	d_cvt_tran_mat2pmat(nx, 1, q, nx, nu+nx, pRSQrq1+(nu+nx)/bs*bs*cnux+(nu+nx)%bs+nu*bs, cnux);
//	d_print_pmat(nu+nx+1, nu+nx, bs, pRSQrq1, cnux);
	...
	d_cvt_mat2pmat(nx, nx, Q, nx, 0, pRSQrqN, cnx);
	d_cvt_tran_mat2pmat(nx, 1, q, nx, nx, pRSQrqN+(nx)/bs*bs*cnx+(nx)%bs, cnx);
...
************************************************/	
	...
	d_zeros_align(&hlam[0], 2*nb_v[0]+2*ng_v[0], 2);
	d_zeros_align(&ht[0], 2*nb_v[0]+2*ng_v[0], 2);
...
//		d_zeros_align(&hpBAbt[ii], pnz_v[ii], cnx_v[ii+1]); for(jj=0; jj<pnz_v[ii]*cnx_v[ii+1]; jj++) hpBAbt[ii][jj] = pBAbt1[jj];
		...
		hb[ii+1] = b;
		hpRSQrq[ii] = pRSQrq1;
//		d_zeros_align(&hpRSQrq[ii], pnz_v[ii], cnux_v[ii]); for(jj=0; jj<pnz_v[ii]*cnux_v[ii]; jj++) hpRSQrq[ii][jj] = pRSQrq1[jj];
		hrq[ii] = rq1;
		hd[ii] = d1;
		hidxb[ii] = idxb1;
		d_zeros_align(&hux[ii], pnux_v[ii], 1);
		d_zeros_align(&hpi[ii+1], pnx_v[ii+1], 1);
		d_zeros_align(&hlam[ii], 2*nb_v[ii]+2*ng_v[ii], 2);
		d_zeros_align(&ht[ii], 2*nb_v[ii]+2*ng_v[ii], 2);
		d_zeros_align(&hrb[ii], pnx_v[ii], 1);
	...
	d_zeros_align(&hlam[N], 2*nb_v[N]+2*ng_v[N], 2);
	d_zeros_align(&ht[N], 2*nb_v[N]+2*ng_v[N], 2);
	d_zeros_align(&hrb[N], pnx_v[N], 1);
...
//		hpmpc_status = d_ip2_res_mpc_hard_tv(&kk, k_max, mu0, mu_tol, alpha_min, warm_start, stat, N, nx_v, nu_v, nb_v, hidxb, ng_v, hpBAbt, hpRSQrq, hpDCt, hd, hux, compute_mult, hpi, hlam, ht, work);
		hpmpc_status = d_tree_ip2_res_mpc_hard_libstr(&kk, k_max, mu0, mu_tol, alpha_min, warm_start, stat, N, nx_v, nu_v, nb_v, hidxb, ng_v, hsBAbt, hsRSQrq, hsDCt, hsd, hsux, compute_mult, hspi, hslam, hst, work);
#else
...
//		d_print_mat(1, 2*nb_v[ii]+2*ng_v[ii], ht[ii], 1);
	...
	double time_final = 0;//(tv3.tv_sec-tv2.tv_sec)/(nrep+0.0)+(tv3.tv_usec-tv2.tv_usec)/(nrep*1e6);
...
************************************************/	
	...
	free(idxb1);
	free(idxbN);
	d_free_align(pRSQrq0);
	d_free_align(pRSQrq1);
	d_free_align(pRSQrqN);
	d_free_align(rq0);
	d_free_align(rq1);
	d_free_align(rqN);
	d_free_align(work);
