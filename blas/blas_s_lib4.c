...
void sgemm_ppp_nt_lib(int m, int n, int k, float *pA, int sda, float *pB, int sdb, float *pC, int sdc, int alg)
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
			kernel_sgemm_pp_nt_8x4_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, alg);
			}
		jj = 0;
		for(; jj<n-j-1; jj+=2)
			{
			kernel_sgemm_pp_nt_8x2_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], bs, alg);
			}
		for(; jj<n-j; jj++)
			{
			kernel_sgemm_pp_nt_8x1_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], &pC[0+(j+jj)*bs+(i+4)*sdc], bs, alg);
			}
		}
	...
	for(; i<m-4; i+=8)
		...
		for(; jj<n-j-1; jj+=2)
			{
			kernel_sgemm_pp_nt_4x2_lib4(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], bs, alg);
			kernel_sgemm_pp_nt_4x2_lib4(k, &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+(i+4)*sdc], bs, alg);
			}
		for(; jj<n-j; jj++)
			{
			kernel_sgemm_pp_nt_4x1_lib4(k, &pA[0+i*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+i*sdc], bs, alg);
			kernel_sgemm_pp_nt_4x1_lib4(k, &pA[0+(i+4)*sda], &pB[jj+j*sdb], &pC[0+(j+jj)*bs+(i+4)*sdc], bs, alg);
			}
		}
#endif
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_sgemm_pp_nt_4x4_lib4(k, &pA[0+i*sda], &pB[0+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, alg);
			}
		jj = 0;
		for(; jj<n-j-1; jj+=2)
			{
...
/*		pB = pB+bs*sdb+bs*bs; // shift to the next block*/
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
			kernel_sgemm_pp_nt_8x4_lib4(n-j-0, &pA[0+(j+0)*bs+i*sda], &pA[0+(j+0)*bs+(i+4)*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 0);
			}
		if(n-j==1)
			{
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_sgemm_pp_nt_4x4_lib4(n-j-0, &pA[0+(j+0)*bs+i*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs, 0);
			}
		if(n-j==1)
			{
			corner_strmm_pp_nt_4x1_lib4(&pA[0+(j+0)*bs+i*sda], &pB[0+(j+0)*bs+j*sdb], &pC[0+(j+0)*bs+i*sdc], bs);
...
void ssyrk_ppp_lib(int m, int n, int k, float *pA, int sda, float *pC, int sdc)
	...
	if(m==n)
		...
		for(; i<m-4; i+=8)
			...
			for(; j<j_end; j+=4)
				{
				kernel_sgemm_pp_nt_8x4_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			if(j<n-3)
				{
				kernel_sgemm_pp_nt_4x4_lib4(k, &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			else if(n-j==1)
				{
				kernel_sgemm_pp_nt_4x1_lib4(k, &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			else if(n-j==2)
				{
				kernel_sgemm_pp_nt_4x2_lib4(k, &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			else if(n-j==3)
				{
				kernel_sgemm_pp_nt_4x3_lib4(k, &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			}
		}
	else
		...
		for(; i<m-4; i+=8)
			...
			if(j<n-3)
				...
				{
				kernel_sgemm_pp_nt_8x1_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			else if(n-j==2)
				{
				kernel_sgemm_pp_nt_8x2_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			else if(n-j==3)
				{
/*				kernel_sgemm_pp_nt_8x4_lib4(k, &pA[0+i*sda], &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);*/
				kernel_sgemm_pp_nt_4x3_lib4(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
				kernel_sgemm_pp_nt_4x3_lib4(k, &pA[0+(i+4)*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+(i+4)*sdc], bs, 1);
				}
			}
		}
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
			kernel_sgemm_pp_nt_4x4_lib4(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
			}
		if(j<n)
			{
			if(n-j==1)
				{
				kernel_sgemm_pp_nt_4x1_lib4(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
				}
			else if(n-j==2)
				{
				kernel_sgemm_pp_nt_4x2_lib4(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
				}
			else if(n-j==3)
				{
				kernel_sgemm_pp_nt_4x3_lib4(k, &pA[0+i*sda], &pA[0+j*sda], &pC[0+(j+0)*bs+i*sdc], bs, 1);
	...
	if(j<n)
		...
		if(n-j==1)
			...
			for(; i<m-4; i+=8)
				{
				kernel_sgemm_pp_nt_4x1_lib4(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
				kernel_sgemm_pp_nt_4x1_lib4(j, &pC[0+(i+4)*sdc], &pC[0+j*sdc], &pC[0+j*bs+(i+4)*sdc], bs, -1);
				}
#endif
			for(; i<m; i+=4)
				{
			...
			kernel_spotrf_strsv_1x1_lib4(m-j-1, &pC[0+j*bs+j*sdc], sdc, info);
			if(*info!=0) return;
			}
		else if(n-j==2)
			{
			...
			for(; i<m-4; i+=8)
				{
				kernel_sgemm_pp_nt_4x2_lib4(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
				kernel_sgemm_pp_nt_4x2_lib4(j, &pC[0+(i+4)*sdc], &pC[0+j*sdc], &pC[0+j*bs+(i+4)*sdc], bs, -1);
				}
#endif
			for(; i<m; i+=4)
				{
			...
			kernel_spotrf_strsv_2x2_lib4(m-j-2, &pC[0+j*bs+j*sdc], sdc, info);
			if(*info!=0) return;
			}
		else if(n-j==3)
			{
...
/*				kernel_sgemm_pp_nt_8x3_lib4(j, &pC[0+i*sdc], &pC[0+(i+4)*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], &pC[0+j*bs+(i+4)*sdc], bs, -1);*/
				kernel_sgemm_pp_nt_4x3_lib4(j, &pC[0+i*sdc], &pC[0+j*sdc], &pC[0+j*bs+i*sdc], bs, -1);
				kernel_sgemm_pp_nt_4x3_lib4(j, &pC[0+(i+4)*sdc], &pC[0+j*sdc], &pC[0+j*bs+(i+4)*sdc], bs, -1);
				}
#endif
			for(; i<m; i+=4)
				{
			...
			kernel_spotrf_strsv_3x3_lib4(m-j-3, &pC[0+j*bs+j*sdc], sdc, info);
			if(*info!=0) return;
			}
		}
...
/*	for(; j<n-(bs-1); j+=bs)*/
	...
	for(; j<n-3; j+=4)
		{
		kernel_sgemv_n_4_lib4(m, pA, x, y, alg);
		pA += sda*bs;
		y  += bs;
		}
	for(; j<n-1; j+=2)
...
void strmv_p_n_lib(int m, int offset, float *pA, int sda, float *x, float *y, int alg)
	...
	if(alg==0 || alg==1)
		...
		for(; j<m-7; j+=8)
			...
			kernel_sgemv_n_4_lib4(4, pA+bs*(j+1), x+j+1, y, 1); // 1 !!!
		...
		for(; j<m-3; j+=4)
			{
			kernel_sgemv_n_4_lib4(j+1, pA, x, y, alg);
			y[1] += pA[1+bs*(j+1)] * x[j+1];
			y[2] += pA[2+bs*(j+1)] * x[j+1] + pA[2+bs*(j+2)] * x[j+2];
			y[3] += pA[3+bs*(j+1)] * x[j+1] + pA[3+bs*(j+2)] * x[j+2] + pA[3+bs*(j+3)] * x[j+3];
			pA += sda*bs;
		...
		for(; j<m-7; j+=8)
			...
			kernel_sgemv_n_4_lib4(4, pA+bs*(j+1), x+j+1, y, -1);
			y[1] -= pA[1+bs*(j+5)] * x[j+5];
			y[2] -= pA[2+bs*(j+5)] * x[j+5] + pA[2+bs*(j+6)] * x[j+6];
			y[3] -= pA[3+bs*(j+5)] * x[j+5] + pA[3+bs*(j+6)] * x[j+6] + pA[3+bs*(j+7)] * x[j+7];
		...
		for(; j<m-3; j+=4)
			{
			kernel_sgemv_n_4_lib4(j+1, pA, x, y, -1);
			y[1] -= pA[1+bs*(j+1)] * x[j+1];
			y[2] -= pA[2+bs*(j+1)] * x[j+1] + pA[2+bs*(j+2)] * x[j+2];
			y[3] -= pA[3+bs*(j+1)] * x[j+1] + pA[3+bs*(j+2)] * x[j+2] + pA[3+bs*(j+3)] * x[j+3];
			pA += sda*bs;
...
void strmv_p_t_lib(int m, int offset, float *pA, int sda, float *x, float *y, int alg)
	...
	if(alg==0 || alg==1)
		...
		if(mna>0)
			...
			for(; j<mna; j++)
				{
				kernel_sgemv_t_1_lib4(mmax-j, mna-j, pA+j*bs+j, sda, x+j, y+j, alg);
				}
			pA += j + (sda-1)*bs + j*bs;
			x  += j;
			y  += j;
		...
		for(; j<m-7; j+=8)
			...
			kernel_sgemv_t_8_lib4(mmax-7, 1, pA+3, sda, x+7, y, alg);
			...
			kernel_sgemv_t_4_lib4(4, 1, pA+3, sda, x+3, y, 1); // !!! 1
		...
		for(; j<m-3; j+=4)
			{
			kernel_sgemv_t_4_lib4(mmax-3, 1, pA+3, sda, x+3, y, alg);
			y[0] += pA[0+bs*0] * x[0] + pA[1+bs*0] * x[1] + pA[2+bs*0] * x[2];
			y[1] += pA[1+bs*1] * x[1] + pA[2+bs*1] * x[2];
			y[2] += pA[2+bs*2] * x[2];
			pA += bs*sda + bs*bs;
		...
		for(; j<m-1; j+=2)
			{
			kernel_sgemv_t_2_lib4(mmax-1, mmax-1, pA+1, sda, x+1, y, alg);
			y[0] += pA[0+bs*0] * x[0];
			pA += 2 + 2*bs;
			x  += 2;
			y  += 2;
		...
		for(; j<m; j++)
			{
			kernel_sgemv_t_1_lib4(mmax, mmax, pA, sda, x, y, alg);
			pA += 1 + bs;
			x  += 1;
			y  += 1;
			mmax -= 1;
		...
		if(mna>0)
			...
			for(; j<mna; j++)
				{
				kernel_sgemv_t_1_lib4(mmax-j, mna-j, pA+j*bs+j, sda, x+j, y+j, -1);
				}
			pA += j + (sda-1)*bs + j*bs;
			x  += j;
			y  += j;
		...
		for(; j<m-7; j+=8)
			...
			kernel_sgemv_t_8_lib4(mmax-7, 1, pA+3, sda, x+7, y, -1);
		...
		for(; j<m-3; j+=4)
			{
			kernel_sgemv_t_4_lib4(mmax-3, 1, pA+3, sda, x+3, y, -1);
			y[0] -= pA[0+bs*0] * x[0] + pA[1+bs*0] * x[1] + pA[2+bs*0] * x[2];
			y[1] -= pA[1+bs*1] * x[1] + pA[2+bs*1] * x[2];
			y[2] -= pA[2+bs*2] * x[2];
			pA += bs*sda + bs*bs;
		...
		for(; j<m-1; j+=2)
			{
			kernel_sgemv_t_2_lib4(mmax-1, mmax-1, pA+1, sda, x+1, y, -1);
			y[0] -= pA[0+bs*0] * x[0];
			pA += 2 + 2*bs;
			x  += 2;
			y  += 2;
		...
		for(; j<m; j++)
			{
			kernel_sgemv_t_1_lib4(mmax, mmax, pA, sda, x, y, -1);
			pA += 1 + bs;
			x  += 1;
			y  += 1;
			mmax -= 1;
...
/*		if(mna%2==1)*/
/*			{*/
/*			kernel_sgemv_n_1_lib4(j, pA, x, y, alg);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] += ptrA[0+bs*0]*ptrx[0];*/
/*			kernel_sgemv_t_1_lib4(m-j-1, 0, ptrA+1, sda, ptrx+1, y, 1);*/
/*			pA += 1;*/
/*			y  += 1;*/
/*			j  += 1;*/
/*			}*/
/*		if(mna%4>=2)*/
/*			{*/
/*			kernel_sgemv_n_2_lib4(j, pA, x, y, alg);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] += ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1];*/
/*			y[1] += ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1];*/
/*			kernel_sgemv_t_2_lib4(m-j-2, 0, ptrA+sda*bs, sda, ptrx+2, y, 1);*/
/*			pA += 2;*/
/*			y  += bs;*/
/*			j  += 2;*/
/*			}*/
...
/*			kernel_sgemv_n_4_lib4(4, ptrA+sda*bs, ptrx, y+4, 1);*/

/*			kernel_sgemv_t_4_lib4(4, 0, ptrA+sda*bs, sda, ptrx+4, y, 1);*/

/*			y[4] += ptrA[0+bs*4+sda*bs]*ptrx[4] + ptrA[1+bs*4+sda*bs]*ptrx[5] + ptrA[2+bs*4+sda*bs]*ptrx[6] + ptrA[3+bs*4+sda*bs]*ptrx[7];*/
/*			y[5] += ptrA[1+bs*4+sda*bs]*ptrx[4] + ptrA[1+bs*5+sda*bs]*ptrx[5] + ptrA[2+bs*5+sda*bs]*ptrx[6] + ptrA[3+bs*5+sda*bs]*ptrx[7];*/
/*			y[6] += ptrA[2+bs*4+sda*bs]*ptrx[4] + ptrA[2+bs*5+sda*bs]*ptrx[5] + ptrA[2+bs*6+sda*bs]*ptrx[6] + ptrA[3+bs*6+sda*bs]*ptrx[7];*/
/*			y[7] += ptrA[3+bs*4+sda*bs]*ptrx[4] + ptrA[3+bs*5+sda*bs]*ptrx[5] + ptrA[3+bs*6+sda*bs]*ptrx[6] + ptrA[3+bs*7+sda*bs]*ptrx[7];*/
/*		*/
/*			kernel_sgemv_t_8_lib4(m-j-8, 0, ptrA+2*sda*bs, sda, ptrx+8, y, 1);*/
/*		*/
/*			pA += 2*sda*bs;*/
/*			y  += 8;*/
/*			}*/
...
/*		for(; j<m-3; j+=4)*/
/*			{*/
/*			kernel_sgemv_n_4_lib4(j, pA, x, y, alg);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] += ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1] + ptrA[2+bs*0]*ptrx[2] + ptrA[3+bs*0]*ptrx[3];*/
/*			y[1] += ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1] + ptrA[2+bs*1]*ptrx[2] + ptrA[3+bs*1]*ptrx[3];*/
/*			y[2] += ptrA[2+bs*0]*ptrx[0] + ptrA[2+bs*1]*ptrx[1] + ptrA[2+bs*2]*ptrx[2] + ptrA[3+bs*2]*ptrx[3];*/
/*			y[3] += ptrA[3+bs*0]*ptrx[0] + ptrA[3+bs*1]*ptrx[1] + ptrA[3+bs*2]*ptrx[2] + ptrA[3+bs*3]*ptrx[3];*/
/*			kernel_sgemv_t_4_lib4(m-j-4, 0, ptrA+sda*bs, sda, ptrx+4, y, 1);*/
/*			pA += 4 + (sda-1)*bs;*/
/*			y  += 4;*/
/*			}*/
/*		for(; j<m-1; j+=2)*/
...
/*			y[0] += ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1];*/
/*			y[1] += ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1];*/
/*			kernel_sgemv_t_2_lib4(m-j-2, 0, ptrA+2, sda, ptrx+2, y, 1);*/
/*			pA += 2;*/
/*			y  += 2;*/
/*			}*/
/*		for(; j<m; j++)*/
...
/*		if(mna%2==1)*/
/*			{*/
/*			kernel_sgemv_n_1_lib4(j, pA, x, y, -1);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] -= ptrA[0+bs*0]*ptrx[0];*/
/*			kernel_sgemv_t_1_lib4(m-j-1, 0, ptrA+1, sda, ptrx+1, y, -1);*/
...
/*		if(mna%4>=2)*/
/*			{*/
/*			kernel_sgemv_n_2_lib4(j, pA, x, y, -1);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] -= ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1];*/
/*			y[1] -= ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1];*/
...
/*		for(; j<m-3; j+=4)*/
/*			{*/
/*			kernel_sgemv_n_4_lib4(j, pA, x, y, -1);*/
/*			ptrA = pA + j*bs;*/
/*			ptrx =  x + j;*/
/*			y[0] -= ptrA[0+bs*0]*ptrx[0] + ptrA[1+bs*0]*ptrx[1] + ptrA[2+bs*0]*ptrx[2] + ptrA[3+bs*0]*ptrx[3];*/
/*			y[1] -= ptrA[1+bs*0]*ptrx[0] + ptrA[1+bs*1]*ptrx[1] + ptrA[2+bs*1]*ptrx[2] + ptrA[3+bs*1]*ptrx[3];*/
...
void strsv_p_t_lib(int n, float *pA, int sda, float *x)
	...
	for(; j<qn-1; j+=2)
		...
		// correct
		kernel_sgemv_t_8_lib4(rn+j*bs, 0, ptrA+2*bs*sda, sda, ptrx+8, ptrx, -1);
