...
/*void dsymv_p_code_generator_old(FILE *f, int m, int offset, int alg)*/
/*	{*/
/*	*/
/*	const int bs = 2;*/
...
fprintf(f, "	\n");
	...
	if(alg==0 || alg==1)
		...
		if(mna==1)
			{
fprintf(f, "	kernel_dsymv_1_lib2(%d, &pA[%d], &x[%d], &y[%d], &x[%d], &y[%d], 1, 1);\n", j, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += 1;
			idxy_n += 1;
			idxx_t += 1;
			idxA  += (sda-1)*bs;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_dsymv_2_lib2(%d, &pA[%d], &x[%d], &y[%d], &x[%d], &y[%d], 1, 1);\n", j, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += sda*bs;
			idxy_n += bs;
			idxx_t += bs;
			}
...
fprintf(f, "	\n");
	...
	if(alg==0 || alg==1)
		...
		if(mna==1)
			{
fprintf(f, "	kernel_dsymv_1_lib2(%d, &pA[%d], &x_n[%d], &y_n[%d], &x_t[%d], &y_t[%d], 0, 1);\n", n, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += 1;
			idxy_n += 1;
			idxx_t += 1;
			idxA  += (sda-1)*bs;
		...
		for(; j<m-1; j+=2)
			{
fprintf(f, "	kernel_dsymv_2_lib2(%d, &pA[%d], &x_n[%d], &y_n[%d], &x_t[%d], &y_t[%d], 0, 1);\n", n, idxA, idxx_n, idxy_n, idxx_t, idxy_t);
			idxA  += sda*bs;
			idxy_n += bs;
			idxx_t += bs;
			}
