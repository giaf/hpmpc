...
/*fprintf(f, "	pB = pB+%d;\n", bs*sdb+bs*bs);*/
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_dgemm_pp_nt_8x4_lib4(%d, &pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
			}
		if(n-j==1)
			{
fprintf(f, "	corner_dtrmm_pp_nt_8x1_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
			}
		else if(n-j==2)
			{
fprintf(f, "	corner_dtrmm_pp_nt_8x2_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
			}
		else if(n-j==3)
			{
fprintf(f, "	corner_dtrmm_pp_nt_8x3_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_dgemm_pp_nt_4x4_lib4(%d, &pA[%d], &pB[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x1_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		else if(n-j==2)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x2_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		else if(n-j==3)
			{
fprintf(f, "	corner_dtrmm_pp_nt_4x3_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		}
...
fprintf(f, "	kernel_dgemm_pp_nt_8x4_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
			...
			if(j<n-3)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x4_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==1)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x1_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==2)
				{
fprintf(f, "	kernel_dgemm_pp_nt_4x2_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==3)
				{
...
fprintf(f, "	kernel_dgemm_pp_nt_8x4_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
				...
				{
fprintf(f, "	kernel_dgemm_pp_nt_8x1_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==2)
				{
fprintf(f, "	kernel_dgemm_pp_nt_8x2_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==3)
				{
fprintf(f, "	kernel_dgemm_pp_nt_8x4_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
...
void dpotrf_p_code_generator(FILE *f, int n, int nna)
	...
	if(j<nna-3)
		{
fprintf(f, "	kernel_dpotrf_dtrsv_4x4_lib4(%d, 4, &pC[%d], %d, info);\n", n-j-4, j*bs+j*sdc, sdc);
...
fprintf(f, "	if(*info!=0) return;\n");
	...
	if(j==0) // assume that n>0
		{
fprintf(f, "	kernel_dpotrf_dtrsv_4x4_lib4(%d, %d, &pC[%d], %d, info);\n", n-j-4, kinv, j*bs+j*sdc, sdc);
fprintf(f, "	if(*info!=0) return;\n");
		kinv = 0;
		j += 4;
		}
...
fprintf(f, "	corner_dpotrf_dtrsv_1x1_lib4(&pC[%d], %d, info);\n", j*bs+j*sdc, sdc);
fprintf(f, "	if(*info!=0) return;\n");
			}
		else if(n-j==2)
			{
...
fprintf(f, "	corner_dpotrf_dtrsv_2x2_lib4(%d, &pC[%d], %d, info);\n", kinv, j*bs+j*sdc, sdc);
fprintf(f, "	if(*info!=0) return;\n");
			}
		else if(n-j==3)
			{
...
fprintf(f, "	corner_dpotrf_dtrsv_3x3_lib4(%d, &pC[%d], %d, info);\n", kinv, j*bs+j*sdc, sdc);
fprintf(f, "	if(*info!=0) return;\n");
			}
		}
...
void dgemv_p_t_code_generator(FILE *f, int n, int m, int offset, int alg)
	...
	for(; j<m-7; j+=8)
		{
fprintf(f, "	kernel_dgemv_t_8_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m-3; j+=4)
		{
fprintf(f, "	kernel_dgemv_t_4_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m-1; j+=2)
		{
fprintf(f, "	kernel_dgemv_t_2_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
	for(; j<m; j++)
		{
fprintf(f, "	kernel_dgemv_t_1_lib4(%d, %d, pA+%d, %d, x, y+%d, %d);\n", n, nna, j*bs, sda, j, alg);
		}
