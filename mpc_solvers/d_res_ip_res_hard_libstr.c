...
void d_res_res_mpc_hard_libstr(int N, int *nx, int *nu, int *nb, int **idxb, int *ng, struct d_strmat *hsBAbt, struct d_strvec *hsb, struct d_strmat *hsRSQrq, struct d_strvec *hsrq, struct d_strvec *hsux, struct d_strmat *hsDCt, struct d_strvec *hsd, struct d_strvec *hspi, struct d_strvec *hslam, struct d_strvec *hst, struct d_strvec *hsres_rq, struct d_strvec *hsres_b, struct d_strvec *hsres_d, struct d_strvec *hsres_m, double *mu, void *work)
	{
	...
	for(ii=0; ii<=N; ii++)
		...
		if(nb0>0)
			...
			dvecad_sp_libstr(nb0, 1.0, idxb[ii], &hswork_0, 0, &hsres_rq[ii], 0);

			dvecex_sp_libstr(nb0, -1.0, &hsux[ii], 0, idxb[ii], &hsres_d[ii], 0);
		...
		if(ii<N)
			...
			daxpy_libstr(nx1, -1.0, &hsux[ii+1], nu1, &hsb[ii], 0, &hsres_b[ii], 0);

			dgemv_nt_libstr(nu0+nx0, nx1, 1.0, 1.0, &hsBAbt[ii], 0, 0, &hspi[ii+1], 0, &hsux[ii], 0, 1.0, 1.0, &hsres_rq[ii], 0, &hsres_b[ii], 0, &hsres_rq[ii], 0, &hsres_b[ii], 0);
			}
