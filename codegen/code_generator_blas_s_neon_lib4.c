...
fprintf(f, "	kernel_spotrf_strsv_4x4_c99_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            	...
	            	for(; j<nna-3; j+=4)
	            		...
	            		for(; i<n-4; i+=8)     
	            			{
fprintf(f, "	kernel_sgemm_pp_nt_8x4_neon_lib4(%d, &pC[%d], &pC[%d], &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, (i+4)*sdc, j*sdc, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
	            			}     
	            		for(; i<n; i+=4)     
	            			{     
fprintf(f, "	kernel_sgemm_pp_nt_4x4_neon_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
	            			}     
fprintf(f, "	kernel_spotrf_strsv_4x4_c99_lib4(%d, &pC[%d], %d);\n", n-j-4, j*bs+j*sdc, sdc);
	            		}     
		}
...
fprintf(f, "	kernel_spotrf_strsv_4x4_c99_lib4(%d, &pC[%d], %d);\n", m-j-4, j*bs+j*sdc, sdc);
	...
	if(j<n)
		...
		if(n-j==1)
			...
			for(; i<m-4; i+=8)
				{
/*fprintf(f, "	kernel_sgemm_pp_nt_8x1_c99_lib4(%d, &pC[%d], &pC[%d], &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, (i+4)*sdc, j*sdc, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	kernel_sgemm_pp_nt_4x1_c99_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
...
fprintf(f, "	kernel_spotrf_strsv_1x1_c99_lib4(%d, &pC[%d], %d);\n", m-j-1, j*bs+j*sdc, sdc);
			...
			for(; i<m-4; i+=8)
				{
/*fprintf(f, "	kernel_sgemm_pp_nt_8x2_c99_lib4(%d, &pC[%d], &pC[%d], &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, (i+4)*sdc, j*sdc, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	kernel_sgemm_pp_nt_4x2_c99_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, i*sdc, j*sdc, j*bs+i*sdc, bs);
fprintf(f, "	kernel_sgemm_pp_nt_4x2_c99_lib4(%d, &pC[%d], &pC[%d], &pC[%d], %d, -1);\n", j, (i+4)*sdc, j*sdc, j*bs+(i+4)*sdc, bs);
				}
			for(; i<m; i+=4)
...
void strmm_ppp_code_generator(FILE *f, int m, int n, int offset)
	...
	for(; i<m-4; i+=8)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_sgemm_pp_nt_8x4_neon_lib4(%d, &pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
			}
		if(n-j==1)
			{
/*fprintf(f, "	corner_strmm_pp_nt_8x1_c99_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	corner_strmm_pp_nt_4x1_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
fprintf(f, "	corner_strmm_pp_nt_4x1_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+(i+4)*sdc, bs);
			}
		else if(n-j==2)
			{
/*fprintf(f, "	corner_strmm_pp_nt_8x2_c99_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	corner_strmm_pp_nt_4x2_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
fprintf(f, "	corner_strmm_pp_nt_4x2_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+(i+4)*sdc, bs);
			}
		else if(n-j==3)
			{
/*fprintf(f, "	corner_strmm_pp_nt_8x3_c99_lib4(&pA[%d], &pA[%d], &pB[%d], &pC[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
...
fprintf(f, "	corner_strmm_pp_nt_4x3_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+(i+4)*sda, j*bs+j*sdb, j*bs+(i+4)*sdc, bs);
	...
	for(; i<m; i+=4)
		...
		for(; j<n-3; j+=4)
			{
fprintf(f, "	kernel_sgemm_pp_nt_4x4_neon_lib4(%d, &pA[%d], &pB[%d], &pC[%d], %d, 0);\n", n-j, j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
			}
		if(n-j==1)
			{
fprintf(f, "	corner_strmm_pp_nt_4x1_c99_lib4(&pA[%d], &pB[%d], &pC[%d], %d);\n", j*bs+i*sda, j*bs+j*sdb, j*bs+i*sdc, bs);
...
/*	const int sdb = PNZ;*/
	...
	if(m==n)
		...
		for(; i<m-4; i+=8)
			...
			for(; j<j_end; j+=4)
				{
fprintf(f, "	kernel_sgemm_pp_nt_8x4_neon_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
				}
			if(j<n-3)
				{
fprintf(f, "	kernel_sgemm_pp_nt_4x4_neon_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==1)
				{
...
fprintf(f, "	kernel_sgemm_pp_nt_8x4_neon_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);
				...
				{
/*fprintf(f, "	kernel_sgemm_pp_nt_8x1_c99_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	kernel_sgemm_pp_nt_4x1_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
fprintf(f, "	kernel_sgemm_pp_nt_4x1_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==2)
				{
/*fprintf(f, "	kernel_sgemm_pp_nt_8x2_c99_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	kernel_sgemm_pp_nt_4x2_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
fprintf(f, "	kernel_sgemm_pp_nt_4x2_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			else if(n-j==3)
				{
/*fprintf(f, "	kernel_sgemm_pp_nt_8x3_neon_lib4(%d, &pA[%d], &pA[%d], &pA[%d], &pC[%d], &pC[%d], %d, 1);\n", k, i*sda, (i+4)*sda, j*sda, j*bs+i*sdc, j*bs+(i+4)*sdc, bs);*/
fprintf(f, "	kernel_sgemm_pp_nt_4x3_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, i*sda, j*sda, j*bs+i*sdc, bs);
fprintf(f, "	kernel_sgemm_pp_nt_4x3_c99_lib4(%d, &pA[%d], &pA[%d], &pC[%d], %d, 1);\n", k, (i+4)*sda, j*sda, j*bs+(i+4)*sdc, bs);
				}
			}
