...
/*void d_update_hessian_box(int k0, int kmax, int nb, int cnz, float sigma_mu, float *t, float *lam, float *lamt, float *dlam, float *bd, float *bl, float *pd, float *pl, float *lb, float *ub)*/
	...
	for(jj=0; jj<N; jj++)
		...
		for(; ii<kmax-7; ii+=8)
			...
			ptr_t_inv[2] = 1.0/ptr_t[2];
			ptr_t_inv[3] = 1.0/ptr_t[3];
			ptr_lamt[2] = ptr_lam[2]*ptr_t_inv[2];
			ptr_lamt[3] = ptr_lam[3]*ptr_t_inv[3];
			ptr_dlam[2] = ptr_t_inv[2]*sigma_mu; // !!!!!
			ptr_dlam[3] = ptr_t_inv[3]*sigma_mu; // !!!!!
			...
			ptr_t_inv[4] = 1.0/ptr_t[4];
			ptr_t_inv[5] = 1.0/ptr_t[5];
			ptr_lamt[4] = ptr_lam[4]*ptr_t_inv[4];
			ptr_lamt[5] = ptr_lam[5]*ptr_t_inv[5];
			ptr_dlam[4] = ptr_t_inv[4]*sigma_mu; // !!!!!
			ptr_dlam[5] = ptr_t_inv[5]*sigma_mu; // !!!!!
			...
			ptr_t_inv[6] = 1.0/ptr_t[6];
			ptr_t_inv[7] = 1.0/ptr_t[7];
			ptr_lamt[6] = ptr_lam[6]*ptr_t_inv[6];
			ptr_lamt[7] = ptr_lam[7]*ptr_t_inv[7];
			ptr_dlam[6] = ptr_t_inv[6]*sigma_mu; // !!!!!
			ptr_dlam[7] = ptr_t_inv[7]*sigma_mu; // !!!!!
			...
			ptr_t_inv[8] = 1.0/ptr_t[8];
			ptr_t_inv[9] = 1.0/ptr_t[9];
			ptr_lamt[8] = ptr_lam[8]*ptr_t_inv[8];
			ptr_lamt[9] = ptr_lam[9]*ptr_t_inv[9];
			ptr_dlam[8] = ptr_t_inv[8]*sigma_mu; // !!!!!
			ptr_dlam[9] = ptr_t_inv[9]*sigma_mu; // !!!!!
			...
			ptr_t_inv[10] = 1.0/ptr_t[10];
			ptr_t_inv[11] = 1.0/ptr_t[11];
			ptr_lamt[10] = ptr_lam[10]*ptr_t_inv[10];
			ptr_lamt[11] = ptr_lam[11]*ptr_t_inv[11];
			ptr_dlam[10] = ptr_t_inv[10]*sigma_mu; // !!!!!
			ptr_dlam[11] = ptr_t_inv[11]*sigma_mu; // !!!!!
			...
			ptr_t_inv[12] = 1.0/ptr_t[12];
			ptr_t_inv[13] = 1.0/ptr_t[13];
			ptr_lamt[12] = ptr_lam[12]*ptr_t_inv[12];
			ptr_lamt[13] = ptr_lam[13]*ptr_t_inv[13];
			ptr_dlam[12] = ptr_t_inv[12]*sigma_mu; // !!!!!
			ptr_dlam[13] = ptr_t_inv[13]*sigma_mu; // !!!!!
			...
			ptr_t_inv[14] = 1.0/ptr_t[14];
			ptr_t_inv[15] = 1.0/ptr_t[15];
			ptr_lamt[14] = ptr_lam[14]*ptr_t_inv[14];
			ptr_lamt[15] = ptr_lam[15]*ptr_t_inv[15];
			ptr_dlam[14] = ptr_t_inv[14]*sigma_mu; // !!!!!
			ptr_dlam[15] = ptr_t_inv[15]*sigma_mu; // !!!!!
