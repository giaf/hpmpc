...
//printf("\n%d\n", ((size_t) ptr) & 63);
		...
		if(ngN>0)
			...
			d_cvt_mat2pmat(nx, ngN, 0, bs, Cf, nx, hpDCt[N], cngN);
			}
        ...
        d_cvt_tran_mat2pmat(nx, nx, 0, bs, Qf, nx, hpQ[N], cnz0);
        for(jj=0; jj<nx; jj++)
...
/*return 1;*/
		...
		if(ngN>0)
			{
			for(ii=0; ii<pnu*cngN; ii++) hpDCt[N][ii] = 0.0; // make sure D is zero !!!!!
			d_cvt_mat2pmat(nx, ngN, nu, bs, Cf, nx, hpDCt[N]+nu/bs*cngN*bs+nu%bs, cngN);
			}
	...
	if(prec=='d')
		...
		for(ii=0; ii<N; ii++)
			...
			if(nx>=nw)
				...
				if(alg==2)
					{
					d_cvt_tran_mat2pmat(nx, nx, 0, bs, Q+ii*nx*nx, nx, hpQRAG[ii], cnwx1);
					}
				else
					{
					...
					d_cvt_tran_mat2pmat(ny, ny, 0, bs, Q+ii*ny*ny, ny, hpQRAG[ii], cnwx1);
					if(alg==0)
						{
						hpCt[ii] = ptr;
						ptr += pnx*cny;
				...
				d_cvt_tran_mat2pmat(nx, nx, 0, bs, A+ii*nx*nx, nx, hpQRAG[ii]+pnx*cnwx1, cnwx1);
				d_cvt_tran_mat2pmat(nw, nw, 0, bs, R+ii*nw*nw, nw, hpQRAG[ii]+(pnx-pnw)*cnwx1+nx*bs, cnwx1);
				d_cvt_tran_mat2pmat(nw, nx, 0, bs, G+ii*nw*nx, nw, hpQRAG[ii]+pnx*cnwx1+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				if(nx>pnx-nx)
					d_cvt_tran_mat2pmat(nx, pnx-nx, nx, bs, A+ii*nx*nx+(nx-pnx+nx)*nx, nx, hpQRAG[ii]+nx/bs*bs*cnwx1+nx%bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nx, nx, nx, bs, A+ii*nx*nx, nx, hpQRAG[ii]+nx/bs*bs*cnwx1+nx%bs, cnwx1);
				if(nx>pnw-nw)
					d_cvt_tran_mat2pmat(nw, pnw-nw, nw, bs, G+ii*nw*nx+(nx-pnw+nw)*nw, nw, hpQRAG[ii]+(pnx-pnw+nw/bs*bs)*cnwx1+nw%bs+nx*bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nw, nx, nw, bs, G+ii*nw*nx, nw, hpQRAG[ii]+(pnx-pnw+nw/bs*bs)*cnwx1+nw%bs+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				}
			else
				{
				if(alg==2)
					{
					d_cvt_tran_mat2pmat(nx, nx, 0, bs, Q+ii*nx*nx, nx, hpQRAG[ii]+(pnw-pnx)*cnwx1, cnwx1);
					}
				else
					{
					for(jj=0; jj<pnx; jj+=4) // loop on panels 
						for(ll=0; ll<nx*bs; ll++) // loop within panels
							hpQRAG[ii][ll+jj*bs*cnwx1] = 0.0;
					d_cvt_tran_mat2pmat(ny, ny, 0, bs, Q+ii*ny*ny, ny, hpQRAG[ii]+(pnw-pnx)*cnwx1, cnwx1);
					if(alg==0)
						{
						hpCt[ii] = ptr;
						ptr += pnx*cny;
				...
				d_cvt_tran_mat2pmat(nx, nx, 0, bs, A+ii*nx*nx, nx, hpQRAG[ii]+pnw*cnwx1, cnwx1);
				d_cvt_tran_mat2pmat(nw, nw, 0, bs, R+ii*nw*nw, nw, hpQRAG[ii]+nx*bs, cnwx1);
				d_cvt_tran_mat2pmat(nw, nx, 0, bs, G+ii*nw*nx, nw, hpQRAG[ii]+pnw*cnwx1+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				if(nx>pnx-nx)
					d_cvt_tran_mat2pmat(nx, pnx-nx, nx, bs, A+ii*nx*nx+(nx-pnx+nx)*nx, nx, hpQRAG[ii]+(pnw-pnx+nx/bs*bs)*cnwx1+nx%bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nx, nx, nx, bs, A+ii*nx*nx, nx, hpQRAG[ii]+(pnw-pnx+nx/bs*bs)*cnwx1+nx%bs, cnwx1);
				if(nx>pnw-nw)
					d_cvt_tran_mat2pmat(nw, pnw-nw, nw, bs, G+ii*nw*nx+(nx-pnw+nw)*nw, nw, hpQRAG[ii]+nw/bs*bs*cnwx1+nw%bs+nx*bs, cnwx1);
				else
					d_cvt_tran_mat2pmat(nw, nx, nw, bs, G+ii*nw*nx, nw, hpQRAG[ii]+nw/bs*bs*cnwx1+nw%bs+nx*bs, cnwx1);
				//d_print_pmat(pnwx1, cnwx1, bs, hpQRAG[ii], cnwx1);
				}
			}
		hpQRAG[N] = ptr;
		...
		if(alg==2)
			{
			d_cvt_tran_mat2pmat(nx, nx, 0, bs, Qf, nx, hpQRAG[N], cnx);
			}
		else // if(alg==0 || alg==1)
			{
			for(jj=0; jj<pnx*cnx; jj++) hpQRAG[N][jj] = 0.0;
			d_cvt_tran_mat2pmat(ny, ny, 0, bs, Qf, ny, hpQRAG[N], cnx);
			if(alg==0)
				{
				hpCt[N] = ptr;
				ptr += pnx*cny;
		...
		if(ndN>0)
			{
			d_cvt_tran_mat2pmat(nx, ndN, 0, bs, D, nx, hpQRAG[N]+pnx*cnx, cnx);
			//d_print_pmat(pnx+pndN, cnx, bs, pQD, cnx);
			if(ndN>pnx-nx)
				d_cvt_tran_mat2pmat(nx, pnx-nx, nx, bs, D+(ndN-pnx+nx)*nx, nx, hpQRAG[N]+nx/bs*bs*cnx+nx%bs, cnx);
			else
				d_cvt_tran_mat2pmat(nx, ndN, nx, bs, D, nx, hpQRAG[N]+nx/bs*bs*cnx+nx%bs, cnx);
			//d_print_pmat(pnx+pndN, cnx, bs, pQD, cnx);
			Ld = ptr;
			ptr += pndN*cndN;
			}
		...
		d_cvt_tran_mat2pmat(nx, nx, 0, bs, L0, nx, hpLe[0], cnx);
		...
		d_cvt_tran_mat2pmat(nx, nx, 0, bs, L0, nx, pL0_inv2, cnx); // XXX
		...
		d_cvt_tran_pmat2mat(nx, nx, 0, bs, hpLe[1], cnx, L0, nx);
		...
		d_cvt_tran_pmat2mat(nx, nx, 0, bs, hpLAG[N], cnx, Le, nx);
