...
************************************************/	
		...
		for(ii=0; ii<=N; ii++)
			...
			for(jj=0; jj<nu[ii]+nx[ii]; jj++)
				{
				tmp = dvecex1_libstr(&hsrrq[ii], jj);
...
************************************************/	
		...
		d_free_strvec(&hsrd[0]);
		d_free_strvec(&hsrm[0]);
		for(ii=1; ii<N; ii++)
			{
